class Digitaltolk::AgentScoreParquetService
  ...
  def arrow_fields
    [
      ...
      Arrow::Field.new("score", :decimal128, scale: 5, precision: 38)
  ...
  def load_columns_data
    ...
    agent_scores.find_in_batches(batch_size: batch_size) do |batch|
      batch.each do |agent_score|
        ...
        @columns['score'] << agent_score.score
        @columns['criteria'] << agent_score.criteria
        @columns['agent_id'] << agent_score.agent_id
  ...
  def map_columns_array
    [
      ...
      Arrow::Decimal128Array.new(@columns['score']),
