...
class CloudwatchReporterConfig
  ...
  def credentials
    ...
    return Aws::InstanceProfileCredentials.new if access_key_id.blank? && secret_access_key.blank?

    # Half a pair either builds a client that fails on every flush or quietly falls back to the
    # role, and both lose the metric with nothing pointing at the cause. Blank counts as missing,
    # since .env files carry empty assignments.
    raise ArgumentError, "#{@env_prefix}AWS_ACCESS_KEY_ID and #{@env_prefix}AWS_SECRET_ACCESS_KEY must be set together" if
