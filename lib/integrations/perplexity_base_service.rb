class Integrations::PerplexityBaseService
  # sonar model supports large context
  # 1 token is approx 4 characters
  # sticking with 120000 to be safe
  # 120000 * 4 = 480,000 characters (rounding off downwards to 400,000 to be safe)
  TOKEN_LIMIT = 400_000
  PERPLEXITY_MODEL = ENV.fetch('PERPLEXITY_MODEL', 'sonar').freeze
  ...
  def cache_key
    ...
    format(::Redis::Alfred::PERPLEXITY_CONVERSATION_KEY, event_name: event_name, conversation_id: conversation.id,
  ...
  def api_url
    endpoint = InstallationConfig.find_by(name: 'CAPTAIN_PERPLEXITY_ENDPOINT')&.value || 'https://api.perplexity.ai/'
  ...
  def make_api_call(body)
    ...
    Rails.logger.info("Perplexity API request: #{body}")
    response = HTTParty.post(api_url, headers: headers, body: body)
    Rails.logger.info("Perplexity API response: #{response.body}")
