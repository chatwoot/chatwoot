...
class Wootql::Parser
  MAX_BYTES = Wootql::Lexer::MAX_BYTES
  MAX_STAGES = 32
  MAX_DEPTH = 16
  ...
  def initialize(source)
    @source = source
    @tokens = Wootql::Lexer.new(source).tokens
    @position = 0
    @conditions = 0
  end
  ...
  def parse
    ...
    stages = []
    while accept('|')
      raise Wootql::Error, 'WootQL exceeds 32 stages' if stages.size >= MAX_STAGES
    ...
    raise e.with_feedback(Wootql::Feedback.location(@source, peek.position),
                          "Current pipeline operation: #{@operation || 'resource selection'}.",
                          context: Wootql::Feedback.stage_context(@operation))
  end
  ...
  def stage
    ...
    arguments = case operation
                ...
                when 'return' then separated { projection }
                when 'join' then join
                when 'summarize' then summary
                when 'sort' then separated { sort_field }
                when 'take' then value
                else raise Wootql::Error, "Unknown WootQL stage: #{operation}"
                end
  ...
  def expression(depth)
    raise Wootql::Error, 'WootQL predicate nesting exceeds 16 levels' if depth > MAX_DEPTH
  ...
  def comparison
    @conditions += 1
    raise Wootql::Error, 'WootQL exceeds 64 predicate terms' if @conditions > 64
    ...
    if keyword('in')
      return { operator: 'in', field: name, value: { parameter: take.value } } if peek.kind == :parameter
    ...
    { operator: operator, field: name, value: comparison_operand }
  end

  def comparison_operand
    return { field: field } if peek.kind == :identifier && !LITERALS.key?(peek.value) && peek.value != 'now'
  ...
  def named_value
    ...
    raise Wootql::Error, "Expected a literal, $parameter, or now() at byte #{peek.position}"
  end
  ...
  def field
    ...
    raise Wootql::Error, 'WootQL field path exceeds 16 segments' if parts.size > MAX_DEPTH
  ...
  def expect(kind)
    accept(kind) || raise(Wootql::Error, "Expected #{kind} at WootQL byte #{peek.position}")
  end

  def expect_keyword(word)
    keyword(word) || raise(Wootql::Error, "Expected #{word} at WootQL byte #{peek.position}")
  end

  def expect_comparison
    return take.value if %w[= != > >= < <=].include?(peek.kind)

    raise Wootql::Error, "Expected comparison at WootQL byte #{peek.position}"
  end
end
