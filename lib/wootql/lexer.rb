...
class Wootql::Lexer
  ...
  def initialize(source)
    raise Wootql::Error, 'WootQL source must be a string of at most 32 KB' unless source.is_a?(String) && source.bytesize <= MAX_BYTES
  ...
  def tokenize(source)
    ...
    until scanner.eos?
      ...
      raise Wootql::Error, 'WootQL exceeds 4096 tokens' if tokens.size >= MAX_TOKENS
  ...
  rescue JSON::ParserError, Wootql::Error => e
    error = e.is_a?(Wootql::Error) ? e : Wootql::Error.new('Invalid WootQL string literal')
    raise error.with_feedback(Wootql::Feedback.location(source, position || scanner.pos))
  end
  ...
  def read_token(scanner)
    ...
    raise Wootql::Error, "Unexpected WootQL token at byte #{scanner.pos}"
  end
