...
function resolveArguments(eventName, properties, options, callback) {
    ...
    var name = validation_1.isPlainObject(eventName) ? eventName.event : eventName;
    if (!name || !validation_1.isString(name)) {
        throw new Error('Event missing');
    }
    var data = validation_1.isPlainObject(eventName)
        ? (_a = eventName.properties) !== null && _a !== void 0 ? _a : {} : validation_1.isPlainObject(properties)
        ? properties
        : {};
    var opts = {};
    if (validation_1.isPlainObject(properties) && !validation_1.isFunction(options)) {
        opts = options !== null && options !== void 0 ? options : {};
    }
    if (validation_1.isPlainObject(eventName) && !validation_1.isFunction(properties)) {
...
function resolvePageArguments(category, name, properties, options, callback) {
    ...
    var objects = args.filter(function (obj) {
        if (resolvedName === null) {
            return validation_1.isPlainObject(obj);
        }
        return validation_1.isPlainObject(obj) || obj === null;
    });
...
exports.resolveUserArguments = function (user) {
    return function () {
        var _a, _b, _c, _d, _e;
        var args = [];
