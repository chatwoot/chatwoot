...
try {
    loc = environment_1.isBrowser() && window.localStorage ? window.localStorage : loc;
}
catch (err) {
...
function persistItems(key, items) {
    ...
    var all = tslib_1.__spreadArrays(items, existing);
    ...
    function PersistedPriorityQueue(maxAttempts, key) {
        ...
        mutex(key, function () {
            try {
                ...
                remove(itemsKey);
                remove(seenKey);
                _this.queue = tslib_1.__spreadArrays(saved, _this.queue);
        ...
        window.addEventListener('beforeunload', function () {
            if (_this.todo > 0) {
                var items_1 = tslib_1.__spreadArrays(_this.queue, _this.future);
                try {
                    mutex(key, function () {
