...
def main():
    run_cmd("Build Containers", 
        "docker compose -f docker-compose.production.vv.yaml build")
    ...
    run_cmd("Preparing Chatwoot DB", 
        "docker compose -f docker-compose.production.vv.yaml run --rm VV-Helpdesk-Rails bundle exec rails db:chatwoot_prepare")
    ...
    run_cmd("Starting containers", 
        "docker compose -f docker-compose.production.vv.yaml up -d")
