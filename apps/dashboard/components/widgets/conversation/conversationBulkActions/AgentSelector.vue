<template>
  <div v-on-clickaway="onCloseAgentList" class="bulk-action__agents">
    ...
    <div class="container">
      <div v-if="assignableAgentsUiFlags.isFetching" class="agent__list-loading">
        <spinner />
      ...
      <div v-else class="agent__list-container">
        <ul v-if="!selectedAgent">
          <li class="search-container">
            <div class="agent-list-search flex-between">
              ...
              <input ref="search" v-model="query" type="search" placeholder="Search" class="agent--search_input" />
            </div>
          </li>
          <li v-for="agent in filteredAgents" :key="agent.id">
            <div class="agent-list-item" @click="assignAgent(agent)">
              <thumbnail :src="agent.thumbnail" :status="agent.availability_status" :username="agent.name" size="22px" />
        ...
        <div v-else class="agent-confirmation-container">
          ...
          <div class="agent-confirmation-actions">
            ...
            <woot-button color-scheme="primary" variant="flat" :is-loading="uiFlags.isUpdating" @click="submit">
              {{ $t('BULK_ACTION.YES') }}
            </woot-button>
          </div>
        </div>
