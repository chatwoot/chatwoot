<template>
  <div v-if="integrationLoaded && !uiFlags.isCreatingSlack" class="flex flex-col flex-1 overflow-auto">
    <div class="bg-white dark:bg-slate-800 border-b border-solid border-slate-75 dark:border-slate-700/50 rounded-sm p-4">
      <integration :integration-id="integration.id" :integration-logo="integration.logo"
        :integration-name="integration.name" :integration-description="integration.description"
        :integration-enabled="integration.enabled" :integration-action="integrationAction"
        :action-button-text="$t('INTEGRATION_SETTINGS.SLACK.DELETE')" :delete-confirmation-text="{
    ...
    <div v-if="areHooksAvailable" class="p-6 flex-1">
      <select-channel-warning v-if="!isIntegrationHookEnabled" :has-connected-a-channel="hasConnectedAChannel" />
      <slack-integration-help-text :selected-channel-name="selectedChannelName" />
    </div>
  </div>
