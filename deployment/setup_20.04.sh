...
function upgrade() {
  cwctl_upgrade_check
  get_cw_version
  echo "Upgrading AlooChat to v$CW_VERSION (branch: $BRANCH)"
...
##############################################################################
# Convert existing AlooChat deployment to different type (--convert)
# Globals:
#   DEPLOYMENT_TYPE
# Arguments:
#   None
...
function convert_deployment() {
  echo "Converting AlooChat deployment to: $DEPLOYMENT_TYPE"

  # Check if AlooChat is installed
  if [ ! -d "/home/chatwoot/chatwoot" ]; then
    echo "AlooChat installation not found. Use --install first."
    exit 1
  fi
