...
required=(
  ...
  VIPS_BLOCK_UNTRUSTED LOCAL_SMOKE SECRET_KEY_BASE
...
for variable in SECRET_KEY_BASE POSTGRES_ADMIN_PASSWORD POSTGRES_PASSWORD REDIS_PASSWORD CLAUDE_AGENT_DATABASE_PASSWORD; do
  value="${!variable}"
  if (( ${#value} < 32 )); then
