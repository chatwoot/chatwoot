...
15. **Rollback, fix, and expand**
   ...
   - Example:
     ...
     ```
   - If blank conversation-message config was filled during migration and needs to be restored, restore only those
     message fields from the stored migration backup:
     ```bash
     bundle exec rake captain:assistant_migration:restore_messages IDS=512,1744 DRY_RUN=false
     ```
