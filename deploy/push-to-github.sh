...
# ---- 2. Copy overlay --------------------------------------------------------
log "Copying engines + deploy + workflows + frontend module ..."
mkdir -p engines deploy app/javascript/dashboard/modules .github/workflows
cp -r "$OVERLAY_DIR/engines/." engines/
cp -r "$OVERLAY_DIR/deploy/." deploy/
# GitHub Actions (image build to GHCR)
if [[ -d "$OVERLAY_DIR/.github/workflows" ]]; then
  cp -r "$OVERLAY_DIR/.github/workflows/." .github/workflows/
fi
