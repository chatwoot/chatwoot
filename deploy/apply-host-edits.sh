...
# apply-host-edits.sh
# -------------------
# Idempotently applies the 6 host-side edits required to wire the
...
#   5. Symlinks engine frontend into app/javascript/dashboard/modules/kanban
#   6. Patches docker/Dockerfile to COPY engines/ before bundle install (path: gem needs it)
...
# ----------------------------------------------------------------------
echo "[6/6] docker/Dockerfile — ensure engines/ is COPY'd before bundle install..."
DOCKERFILE="docker/Dockerfile"
if [[ ! -f "$DOCKERFILE" ]]; then
  echo "    WARN: $DOCKERFILE not found — skipping (only needed for docker build)."
elif grep -qE "^COPY engines/ engines/" "$DOCKERFILE"; then
  echo "    already patched."
else
  # Multi-stage Dockerfile copies Gemfile + Gemfile.lock, then bundle installs,
  # then COPY . . . But the `gem 'chatwoot_kanban', path: 'engines/chatwoot_kanban'`
  # needs the engine dir at bundle-install time. Inject COPY engines/ engines/
  # right after every `COPY Gemfile` line (multi-stage builds may have several).
  perl -i -pe 's{(^COPY Gemfile.*\n)}{$1COPY engines/ engines/\n}g' "$DOCKERFILE"
  echo "    patched (COPY engines/ inserted after COPY Gemfile)."
fi

echo
echo "DONE. Review with: git diff Gemfile config/routes.rb app/javascript/dashboard/ docker/Dockerfile"
