...
Other than declared props the `woot-button` can be customised by providing custom classnames through `class-names` prop.
...
There are different styles that comes with chatwoot buttons. This can set with `variants` prop for `woot-button`. By deault the filled will be the style, other variations are `smooth, hollow, clear, link`
...
The button can be disabled with `is-disabled` which accepts a boolean value. The button can set to a loading state with `is-loading` which accepts a boolean value.
