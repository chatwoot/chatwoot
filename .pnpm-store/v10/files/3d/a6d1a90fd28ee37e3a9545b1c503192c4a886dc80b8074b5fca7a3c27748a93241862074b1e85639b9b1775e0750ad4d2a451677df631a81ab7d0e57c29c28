...
exports.allowedCssProperties = exports.sanitize = void 0;
...
function sanitizeHtml(input, { dropAllHtmlTags = false, rewriteExternalLinks, rewriteExternalResources, id = 'msg_' +
    String.fromCharCode(...new Array(24)
        .fill(undefined)
        .map(() => ((Math.random() * 25) % 25) + 65)), allowedSchemas = defaultAllowedSchemas, allowedCssProperties = constants_js_1.allowedCssProperties, preserveCssPriority = true, noWrapper = false, }) {
    if (noWrapper)
        id = '';
    ...
    const removeTags = [...constants_js_1.removeWithContents];
    if (dropAllHtmlTags) {
    ...
    while ((node = elementIter.nextNode())) {
        ...
        if (tagName in constants_js_1.allowedTags) {
            const allowedAttributes = constants_js_1.allowedTags[tagName];
...
exports.allowedCssProperties = constants_js_1.allowedCssProperties;
