...
| **P2 — Widget revamp** | تم JSON، ۸ آیکون دیفالت، motion/sound، voice note، dept picker، offline form، RTL | 🔶 **مرحله ۱ انجام شد (2026-08-23):** `packages/widget-theme` با schema + ۸ تم + ۸ آیکون + motion presets؛ اتصال به SDK از طریق `chatwootSettings.theme` (ایزوله `[whisker]`)؛ صفحه دمو ۳ تم از یک build؛ RTL ویجت از قبل فعال. **مرحله ۲ انجام شد (2026-08-23):** voice note (MediaRecorder + mic button + audio upload)؛ audio playback برای پیام‌های کاربر؛ notification sounds (Web Audio API beep هنگام پیام جدید اپراتور)؛ offline form با مسیر `/offline` و فرم name/email/message. *باقیمانده: department picker (نیازمند API backend برای Teams/Departments)، انتخاب sound pack سمت ویجت* |
| **P3 — Goftino parity backend** | worktime، ondelay، ban rules، rating/NPS، export CSV، SMS adapter، Bale channel، auto-distribution queue | 🔶 **انجام شد (2026-08-24):** Bale channel کامل (model/migration/webhook/services/dashboard form/i18n en+fa)؛ CSAT/Rating از قبل موجود؛ CSV Export (مخاطبین + گزارش‌ها)؛ Auto-distribution round-robin؛ Block/Mute موجود. *باقیمانده: SMS adapter HTTP جنریک (Kavenegar/SMS.ir)، Ban list UI اختصاصی* |
| **P4 — AI layer** | whisker_ai engine، provider UI، AI Agent + RAG(pgvector)، copilot، widget AI toggle | ✅ **انجام شد (2026-08-24):** BYOR engine + provider resolver wired into Captain + widget AI auto-reply toggle + RAG knowledge base با pgvector. *باقیمانده: copilot depth* |
| **P5 — Pet** | Tauri app + widget pet-mode | ✅ **انجام شد (2026-08-24):** `apps/pet/` — Tauri app با transparent window، Pawly SVG blob، drag، notification badge، context menu، **system tray integration**، **API polling** (HTTP client + unread count + desktop notifications)، settings panel (URL + token). *باقیمانده: build ویندوز/مک/لینوکس، system tray icons* |
| **P6 — Flow builder & launch** | flow editor، marketplace تم، docs site، demo online | 🔶 **مرحله ۲ انجام شد (2026-08-24):** Visual flow editor — `FlowBuilderEditor.vue` + `FlowCanvas.vue` (drag-n-drop nodes، bezier edges، pan/zoom) + `NodeProperties.vue` (per-type config)؛ `ThemeMarketplace.vue` + Vuex store + API； `docs/index.html` docs site scaffold با sidebar navigation. *باقیمانده: theme marketplace backend routes، visual node editor polish (snap-to-grid, undo/redo)* |
