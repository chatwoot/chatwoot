...
| **P4 — AI layer** | whisker_ai engine، provider UI، AI Agent + RAG(pgvector)، copilot، widget AI toggle | 🔶 **مرحله ۱ انجام شد (2026-08-24):** `engines/whisker_ai/` با WhiskerAi::Provider model (BYOR: per-account multi-provider، encrypted API key، primary/fallback)؛ ProviderResolver (routing per feature با fallback chain)؛ API CRUD + set_primary؛ dashboard settings page (AiProviders.vue) با افزودن/ویرایش/حذف/انتخاب اصلی؛ Vuex store + API client. *باقیمانده: اتصال ProviderResolver به Captain::BaseTaskService، RAG با pgvector، widget AI toggle، copilot integration* |
