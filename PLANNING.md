...
| **P6 — Flow builder & launch** | flow editor، marketplace تم، docs site، demo online | 🔶 **مرحله ۱ انجام شد (2026-08-24):** `ConversationFlow` model + migration + `FlowExecutionService` (trigger/condition/action/AI reply/send message nodes)؛ API CRUD + toggle؛ `FlowBuilder.vue` dashboard UI با node editor؛ Vuex store. *باقیمانده: visual node editor (drag & drop)، marketplace تم، docs site* |
