...
| **P4 — AI layer** | whisker_ai engine، provider UI، AI Agent + RAG(pgvector)، copilot، widget AI toggle | 🔶 **مرحله ۱، ۲ و ۳ انجام شد (2026-08-24):** `engines/whisker_ai/` با WhiskerAi::Provider model (BYOR: per-account multi-provider، encrypted API key، primary/fallback)؛ ProviderResolver (routing per feature با fallback chain)؛ API CRUD + set_primary؛ dashboard settings page (AiProviders.vue)؛ **wired into Captain::BaseTaskService** (llm_credential، api_base، make_api_call همگی BYOR را اولویت می‌دهند)؛ **widget AI auto-reply toggle** (migration ai_auto_reply، AiAutoReplyService، AiAutoReplyListener، AiAssistantBadge، dashboard toggle، i18n en+fa). *باقیمانده: RAG با pgvector، copilot depth* |
