...
export const actions = {
  ...
  acceptCall({ commit, dispatch, rootGetters }, call_data) {
    if (state.activeCall) {
      const messages = rootGetters['conversation/getConversation'];

      const message = Object.values(toRaw(messages))
        .reverse()
        .find(e => e.content_attributes.call_room === call_data.room_id);
  ...
  rejectCall({ dispatch, rootGetters }, call_data) {
    if (state.activeCall) {
      const messages = rootGetters['conversation/getConversation'];
