<template>
  <div class="phone-input--wrap relative inline-block">
    <div
      class="flex items-center dark:bg-slate-900 justify-start rounded-md border border-solid h-8"
      :class="
        error
    ...
    >
      <div
        class="cursor-pointer py-2 pr-1.5 pl-2 rounded-tl-md rounded-bl-md flex items-center justify-center gap-1.5 bg-slate-25 dark:bg-slate-700 h-7 w-14"
      ...
      <span
        v-if="activeDialCode"
        class="flex bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-normal text-base leading-normal py-0.5 pl-1 pr-0 h-7"
      >
        {{ activeDialCode }}
      </span>
      <input
    ...
    <div class="absolute z-[100000]">
      <div
        ...
        tabindex="0"
        class="fixed h-60 w-[12.5rem] shadow-md overflow-y-auto rounded px-0 pt-0 pb-1 bg-white dark:bg-slate-900"
