...
const standardVariables = computed(() =>
  MESSAGE_VARIABLES.filter(variable => {
    if (
      props.context === 'campaign' &&
    ...
    return (
      variable.label.toLowerCase().includes(searchTerm.value) ||
      variable.key.toLowerCase().includes(searchTerm.value)
    );
  ...
  customAttributes.value
    .filter(attribute => {
      if (attribute.attribute_model === 'conversation_attribute') {
        return props.context !== 'campaign';
      }
      return CUSTOM_ATTRIBUTE_PREFIXES[attribute.attribute_model];
    })
    .filter(
      attribute =>
        `${CUSTOM_ATTRIBUTE_PREFIXES[attribute.attribute_model]}.custom_attribute.${attribute.attribute_key}`
          .toLowerCase()
...
const items = computed(() =>
  [...standardVariables.value, ...customVariables.value].map(
    ({ key, description }) => ({
      id: key,
      key,
      label: `{{${key}}}`,
