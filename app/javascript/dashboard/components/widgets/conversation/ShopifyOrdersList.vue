...
<template>
  <div class="px-4 py-2 text-s-primary">
    <div v-if="!hasSearchableInfo" class="text-center text-s-primary">
    ...
    <div v-else-if="error" class="text-center text-s-error-text">
      {{ error }}
    </div>
    <div v-else-if="!orders.length" class="text-center text-s-primary">
      {{ $t('CONVERSATION_SIDEBAR.SHOPIFY.NO_SHOPIFY_ORDERS') }}
    </div>
    <div v-else>
      <ShopifyOrderItem
