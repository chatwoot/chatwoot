...
const error = ref('');
const obsInternas = ref('');

const handleConfirm = async () => {
  isCreating.value = true;
  error.value = '';
  try {
    await ViariAPI.criarReserva({
      clienteId: props.viariClienteId,
      orcamentoId: props.orcamento.id,
      obsInternas: obsInternas.value,
    });
    emit('created');
    emit('close');
  } catch (e) {
    ...
    >
      ...
      >
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm font-bold text-[#5DCAA5]">{{
            $t('CONVERSATION_SIDEBAR.VIARI.RESERVA_MODAL.TITLE')
          }}</span>
          <span
      ...
      <div class="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        <div class="rounded-lg p-3 bg-[#E1F5EE]">
          ...
          >
            {{ $t('CONVERSATION_SIDEBAR.VIARI.RESERVA_MODAL.ORCAMENTO_REF') }}
          </div>
          <div class="text-sm font-bold text-[#0D2B2A]">
            {{ orcamento.codigo }}
          </div>
          <div class="flex gap-3 text-xs text-[#0F6E56] mt-0.5">
            <span>
            ...
            <span>
              {{ $t('CONVERSATION_SIDEBAR.VIARI.RESERVA_MODAL.CARTAO_LABEL') }}
        ...
        <div>
          ...
          >
            {{ $t('CONVERSATION_SIDEBAR.VIARI.RESERVA_MODAL.OBS_LABEL') }}
          </label>
          <textarea
            v-model="obsInternas"
            rows="3"
      ...
      >
        ...
        >
          ...
          {{
            isCreating
              ? $t('CONVERSATION_SIDEBAR.VIARI.RESERVA_MODAL.CREATING')
              : $t('CONVERSATION_SIDEBAR.VIARI.RESERVA_MODAL.CONFIRM')
          }}
        </button>
      </div>
    </div>
