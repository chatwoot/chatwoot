...
<template>
  <div>
    <ul
      v-if="items.length"
      ref="tagAgentsRef"
      class="vertical dropdown menu mention--box bg-s-surface p-1 rounded-xl text-sm overflow-auto absolute w-full z-20 shadow-md left-0 leading-[1.2] bottom-full max-h-[12.5rem] border border-solid border-s-border-strong"
      role="listbox"
    >
      <li
        v-for="item in items"
      ...
      >
        ...
        <div
          ...
          class="px-2 py-2 text-xs font-medium tracking-wide capitalize text-s-muted"
        >
          {{ item.title }}
        </div>
        <!-- Selectable Item -->
        <div
          v-else
          :class="{
            'bg-s-primary/15': getSelectableIndex(item) === selectedIndex,
          }"
        ...
        >
          ...
          >
            <h5
              class="overflow-hidden mb-0 text-sm capitalize whitespace-nowrap text-s-muted text-ellipsis"
              :class="{
                'text-s-primary': getSelectableIndex(item) === selectedIndex,
              }"
            >
              {{ item.displayName }}
            </h5>
            <div
              class="overflow-hidden text-xs whitespace-nowrap text-ellipsis text-s-muted"
              :class="{
                'text-s-muted': getSelectableIndex(item) === selectedIndex,
              }"
            >
              {{ item.displayInfo }}
            </div>
