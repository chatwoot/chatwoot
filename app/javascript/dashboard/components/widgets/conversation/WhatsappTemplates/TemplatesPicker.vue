...
<template>
  <div class="w-full">
    <div class="flex gap-2 mb-2.5">
      <div
        class="flex flex-1 gap-1 items-center px-2.5 py-0 rounded-lg bg-s-primary/15 outline outline-1 outline-s-border hover:outline-s-border dark:hover:outline-s-border focus-within:outline-s-brand dark:focus-within:outline-s-brand"
      >
        <fluent-icon icon="search" class="text-s-primary" size="16" />
        <input
          v-model="query"
          type="search"
          :placeholder="t('WHATSAPP_TEMPLATES.PICKER.SEARCH_PLACEHOLDER')"
          class="reset-base w-full h-9 bg-transparent text-s-primary !text-sm !outline-0"
        />
      </div>
      <button
        :disabled="isRefreshing"
        class="flex justify-center items-center w-9 h-9 rounded-lg bg-s-primary/15 outline outline-1 outline-s-border hover:outline-s-border dark:hover:outline-s-border hover:bg-s-subtle dark:hover:bg-s-subtle disabled:opacity-50 disabled:cursor-not-allowed"
        :title="t('WHATSAPP_TEMPLATES.PICKER.REFRESH_BUTTON')"
        @click="refreshTemplates"
      >
        <Icon
          icon="i-lucide-refresh-ccw"
          class="text-s-primary size-4"
          :class="{ 'animate-spin': isRefreshing }"
        />
      </button>
    </div>
    <div
      class="bg-s-bg outline-s-border outline outline-1 rounded-lg max-h-[18.75rem] overflow-y-auto p-2.5"
    >
      <div v-for="(template, i) in filteredTemplateMessages" :key="template.id">
        <button
          class="block p-2.5 w-full text-left rounded-lg cursor-pointer hover:bg-s-subtle dark:hover:bg-s-subtle"
        ...
        >
          <div>
            <div class="flex justify-between items-center mb-2.5">
              ...
              <span
                class="inline-block px-2 py-1 text-xs leading-none rounded-lg cursor-default bg-s-subtle text-s-primary"
              >
            ...
            <div v-if="getTemplateHeader(template)" class="mb-3">
              <p class="text-xs font-medium text-s-muted">
              ...
              <div
                ...
                class="text-sm italic text-s-muted"
              >
                {{
            ...
            <div v-if="getTemplateButtons(template)" class="mt-3">
              ...
              <div class="flex flex-wrap gap-1 mt-1">
                <span
                  ...
                  :key="button.text"
                  class="px-2 py-1 text-xs rounded bg-s-subtle text-s-primary"
                >
                  {{ button.text }}
                </span>
              </div>
        ...
        <hr
          ...
          :key="`hr-${i}`"
          class="border-b border-solid border-s-border my-2.5 mx-auto max-w-[95%]"
        />
      </div>
