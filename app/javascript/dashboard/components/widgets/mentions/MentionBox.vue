<template>
  ...
  >
    <ul class="vertical dropdown menu">
      ...
      >
        <button
          class="flex group gap-0.5 overflow-hidden cursor-pointer rounded-none py-2.5 px-2.5 items-center w-full h-full text-left hover:bg-woot-50 dark:hover:bg-woot-800 border-x-0 border-t-0 border-b border-solid border-slate-100 dark:border-slate-700"
          :class="{
            'bg-woot-25 dark:bg-woot-800': index === selectedIndex,
          }"
          @click="onListItemSelection(index)"
        >
          <div class="flex-1 w-[calc(100%-40px)] flex flex-col">
            <p
          ...
          <thumbnail
            ...
            size="36px"
            class="flex-none"
            :txt-badge="`SL:${item.images.length}`"
          />
        </button>
      </woot-dropdown-item>
    </ul>
