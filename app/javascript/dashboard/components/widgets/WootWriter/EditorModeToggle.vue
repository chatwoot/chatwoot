...
<template>
  <button
    class="flex items-center w-auto h-8 p-1 transition-all border rounded-full bg-n-alpha-2 group relative duration-300 ease-in-out z-0 active:scale-[0.995] active:duration-75 text-sm font-medium"
    :disabled="disabled || isReplyRestricted"
    :class="{
      'cursor-not-allowed': disabled || isReplyRestricted,
      'border-outline-variant/[.15]': !isPrivate,
      'border-n-amber-8/20': isPrivate,
    }"
    @click="$emit('toggleMode')"
  >
    <div
      ref="wootEditorReplyMode"
      class="flex items-center gap-1 px-2 z-20 transition-colors duration-200"
      :class="isPrivate ? 'text-on-surface-variant/60' : 'text-on-surface'"
    >
    ...
    <div
      ...
      class="flex items-center gap-1 px-2 z-20 transition-colors duration-200"
      :class="isPrivate ? 'text-n-amber-11' : 'text-on-surface-variant/60'"
    >
    ...
    <div
      ...
      :class="{
        ...
        'bg-surface-container-high': !isPrivate,
        'bg-n-amber-3': isPrivate,
      }"
      :style="{
        '--chip-width': width,
