...
<template>
  <div class="relative phone-input--wrap">
    <div
      class="flex items-center justify-start border-none outline outline-1 rounded-lg bg-s-primary/15"
      :class="
        error
          ? 'outline-s-error dark:outline-s-error hover:outline-s-error dark:hover:outline-s-error mb-1'
          : 'mb-4 outline-s-border dark:outline-s-border hover:outline-s-border dark:hover:outline-s-border'
      "
    >
      <div
        class="cursor-pointer py-2 pr-1.5 pl-2 rounded-tl-lg rounded-bl-lg flex items-center justify-center gap-1.5 bg-s-subtle h-10 w-14"
      ...
      <span
        v-if="activeDialCode"
        class="flex py-2 ltr:pl-2 rtl:pr-2 text-base font-normal leading-normal text-s-primary"
      >
        {{ activeDialCode }}
      </span>
      <input
    ...
    <div
      ...
      tabindex="0"
      class="z-10 absolute h-60 w-[12.5rem] shadow-md overflow-y-auto top-10 rounded-lg px-0 pt-0 pb-1 bg-s-subtle backdrop-blur-[100px]"
    ...
    >
      ...
      >
        <input
          ...
          class="!h-8 !mb-0 !text-sm !outline-s-brand dark:!outline-s-brand"
      ...
      <div
        ...
        ref="dropdownItem"
        :key="index"
        class="flex items-center px-1 py-0 cursor-pointer h-7 hover:bg-s-subtle dark:hover:bg-s-subtle"
        :class="{
          'bg-s-subtle dark:bg-s-subtle': country.id === activeCountryCode,
          'bg-s-subtle dark:bg-s-subtle': index === selectedIndex,
        }"
      ...
      >
        ...
        <span class="ml-1 text-xs text-s-muted">
          {{ country.dial_code }}
        </span>
      </div>
      <div v-if="filteredCountriesBySearch.length === 0">
        <span
          class="flex items-center justify-center mt-4 text-sm text-s-muted"
        >
