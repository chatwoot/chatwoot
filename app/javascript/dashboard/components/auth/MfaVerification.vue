  ...
  <div class="w-full max-w-md mx-auto">
    ...
    >
      ...
      <div class="text-center mb-6">
        <div
          class="inline-flex items-center justify-center size-14 bg-s-surface outline outline-s-border rounded-full mb-4"
        >
          <Icon icon="i-lucide-lock-keyhole" class="size-6 text-s-muted" />
        </div>
        <h2 class="text-2xl font-semibold text-s-primary">
          {{ $t('MFA_VERIFICATION.TITLE') }}
        </h2>
        <p class="text-sm text-s-muted mt-2">
          {{ $t('MFA_VERIFICATION.DESCRIPTION') }}
        </p>
      </div>

      <!-- Tab Selection -->
      <div class="flex rounded-lg bg-s-primary/15 p-1 mb-6">
        <button
          ...
          :class="
            verificationMethod === method
              ? 'bg-s-brand-soft text-s-primary shadow-sm'
              : 'text-s-primary'
          "
      ...
      <form class="space-y-4" @submit.prevent="handleVerification">
        ...
        <div v-if="verificationMethod === OTP">
          <label class="block text-sm font-medium text-s-primary mb-2">
          ...
          <div class="flex justify-between gap-2">
            <input
              ...
              pattern="[0-9]"
              inputmode="numeric"
              class="w-12 h-12 text-center text-lg font-semibold border-2 border-s-border hover:border-s-border-strong rounded-lg focus:border-s-brand bg-s-primary/15 text-s-primary placeholder:text-s-muted"
        ...
        <div
          v-if="errorMessage"
          class="p-3 bg-s-error-soft outline outline-s-error/30 outline-1 rounded-lg"
        >
          <p class="text-sm text-s-error">{{ errorMessage }}</p>
        </div>
    ...
    <Dialog
      ...
      class="[&>dialog>div]:bg-s-subtle [&>dialog>div]:rounded-lg"
      @confirm="helpModalRef?.close()"
    >
      <div class="space-y-4 text-sm text-s-muted">
        <div v-for="section in ['AUTHENTICATOR', 'BACKUP']" :key="section">
          <h4 class="font-medium text-s-primary mb-2">
