<template>
  <div class="flex-1 overflow-auto p-4">
    ...
    <div class="flex flex-row gap-4">
      <div class="w-[60%]">
        ...
        >
          ...
          <router-link v-if="isAdmin" :to="addAccountScoping('settings/chatbot/create')">
            {{ $t('CHATBOT_SETTINGS.NEW_BOT') }}
          </router-link>
        </p>
            ...
            <tr v-for="chatbot in chatbotsList" :key="chatbot.chatbot_id">
              <td>
                <span class="agent-name">
                  <template v-if="chatbot.chatbot_name === null || chatbot.chatbot_name === ''">
                    <span class="chatbot-id">{{ $t('CHATBOT_SETTINGS.CHATBOT') }} - {{ chatbot.chatbot_id.slice(-4) }}</span>
                  </template>
                  <template v-else>
              ...
              <td>
                <div class="button-wrapper">
                  <router-link
                    :to = "addAccountScoping(`settings/chatbot/edit/${chatbot.chatbot_id}@${chatbot.last_trained_at}`)"
                  >
                    <woot-button
                      v-if="isAdmin"
    ...
    <woot-confirm-delete-modal
      ...
      :confirm-value="selectedChatbot.chatbot_name || selectedChatbot.chatbot_id"
...
export default {
  ...
  computed: {
    ...
    confirmDeleteTitle() {
      return this.$t('CHATBOT_SETTINGS.DELETE.CONFIRM.TITLE', {
        name: this.selectedChatbot.chatbot_name || this.selectedChatbot.chatbot_id,
      });
    },
  ...
  async created() {
    const res = await ChatbotAPI.fetchChatbotsWithAccountId(this.currentAccountId);
