    ...
    <form class="row" @submit.prevent="createChannel()">
      <div class="medium-8 columns">
        <label :class="{ error: $v.channelName.$error }">
          {{ $t('INBOX_MGMT.ADD.WHATSAPP.CHANNEL_NAME.LABEL') }}
          <input
            v-model.trim="channelName"
            type="text"
            :placeholder="
              $t('INBOX_MGMT.ADD.WHATSAPP.CHANNEL_NAME.PLACEHOLDER')
            "
            @blur="$v.channelName.$touch"
          />
          <span v-if="$v.channelName.$error" class="message">{{
            $t('INBOX_MGMT.ADD.WHATSAPP.CHANNEL_NAME.ERROR')
          }}</span>
        </label>
      </div>
      ...
      <div class="medium-8 columns">
        <label :class="{ error: $v.accountSID.$error }">
          {{ $t('INBOX_MGMT.ADD.WHATSAPP.ACCOUNT_SID.LABEL') }}
          <input
            v-model.trim="accountSID"
            type="text"
            :placeholder="$t('INBOX_MGMT.ADD.WHATSAPP.ACCOUNT_SID.PLACEHOLDER')"
            @blur="$v.accountSID.$touch"
          />
          <span v-if="$v.accountSID.$error" class="message">{{
            $t('INBOX_MGMT.ADD.WHATSAPP.ACCOUNT_SID.ERROR')
          }}</span>
        </label>
      </div>
      <div class="medium-8 columns">
        <label :class="{ error: $v.authToken.$error }">
          {{ $t('INBOX_MGMT.ADD.WHATSAPP.AUTH_TOKEN.LABEL') }}
          <input
            v-model.trim="authToken"
            type="text"
            :placeholder="$t('INBOX_MGMT.ADD.WHATSAPP.AUTH_TOKEN.PLACEHOLDER')"
            @blur="$v.authToken.$touch"
          />
          <span v-if="$v.authToken.$error" class="message">{{
            $t('INBOX_MGMT.ADD.WHATSAPP.AUTH_TOKEN.ERROR')
          }}</span>
        </label>
      </div>
