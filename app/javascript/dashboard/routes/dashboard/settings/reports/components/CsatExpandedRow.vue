...
<template>
  <div class="py-4 px-5 border-t border-s-border bg-s-bg">
    <CsatReviewNotesPaywall v-if="showPaywall" />
    <div v-else-if="isFeatureEnabled" class="flex flex-col gap-3">
      <div class="flex items-start gap-4">
        <div
          class="flex items-center gap-1.5 text-s-muted shrink-0 w-36 pt-3"
        >
        ...
        <div class="flex-1 max-w-2xl">
          <div
            ...
            class="group flex items-start gap-2 py-2 px-3 rounded-lg hover:bg-s-subtle dark:hover:bg-s-subtle cursor-pointer transition-colors"
            @click.stop="startEditing"
          >
            <p
              v-dompurify-html="formatMessage(response.csat_review_notes || '')"
              class="flex-1 text-sm text-s-primary prose-sm prose-p:text-sm prose-p:leading-relaxed prose-p:mb-1 prose-p:mt-0"
            />
            <i
              class="i-lucide-pencil size-4 text-s-muted opacity-0 group-hover:opacity-100 transition-opacity shrink-0 mt-0.5"
            />
          </div>
      ...
      >
        <div class="flex items-center gap-1.5 text-s-muted shrink-0 w-36">
