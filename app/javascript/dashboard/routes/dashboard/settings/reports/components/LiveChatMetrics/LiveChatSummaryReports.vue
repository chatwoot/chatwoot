...
export default {
  ...
  computed: {
    summaryData() {
      const titlesMapping = {
        ...
        pre_sale_queries: 'Pre sales conversations',
        live_chat_orders_placed: 'Total Orders',
        live_chat_revenue_generated: 'Revenue Generated',

        // Support Analytics metrics
        bot_handled: 'Total conversations',
      ...
      Object.keys(groupedData).forEach(key => {
        ...
        if (key === 'live_chat_orders_placed') {
  ...
  methods: {
    renderContent(key, value, total) {
      ...
      const financeMetrics = ['live_chat_revenue_generated'];
    ...
    async fetchAllData() {
      ...
      try {
        await Promise.all([
          this.$store.dispatch('summaryReports/fetchLiveChatCurrency'),
