...
<template>
  <div
    class="grid items-center content-center w-full h-16 grid-cols-12 gap-4 px-6 py-0 border-b last:border-b-0 last:rounded-b-xl border-s-border"
  >
    <div
      class="flex items-center gap-2 col-span-6 px-0 py-2 text-sm tracking-[0.5] text-s-primary rtl:text-right"
    >
      <router-link :to="routerParams" class="text-s-primary hover:underline">
        {{ `#${conversationId}` }}
      </router-link>
      <span class="text-s-muted">
        {{ $t('SLA_REPORTS.WITH') }}
      </span>
      <span class="capitalize truncate text-s-primary">{{
    ...
    <div
      class="flex items-center capitalize py-2 px-0 text-sm tracking-[0.5] text-s-primary text-left rtl:text-right col-span-2"
    >
      {{ slaName }}
    </div>
    <div class="flex items-center col-span-2 gap-2">
      ...
      />
      <span v-else class="text-s-muted"> --- </span>
    </div>
