...
const agentHandoff = computed(() => {
    const aiAgentHandoffCount = isFinite(props.metrics.aiAgentHandoffCount) ? props.metrics.aiAgentHandoffCount : 0
    const handoffRate = isFinite(props.metrics.handoffRate) ? props.metrics.handoffRate : 0
    ...
    return {
        ...
        handoffRatePercentage: formatToPercent(isFinite(handoffRateToHuman) ? handoffRateToHuman : 0),
...
<template>
    <div
        ...
        <div class="p-4 mb-3 rounded-md">
            <ChartStats2 :title="$t('REPORT.METRICS.AI_MESSAGE_USAGE.NAME')" :desc="`${aiUsage.averageUsage}`"
                :legend1="aiUsage.data1.label" :legend1Value="`${aiUsage.totalCreditUsage}`"
                :legend2="aiUsage.data2.label" :legend2Value="`${aiUsage.totalAiMessageSend}`" />
            <div class="mt-4 h-72">
        ...
        <div class="p-4 mb-3 rounded-md">
            <ChartStats2 :title="$t('REPORT.METRICS.AGENT_HANDOFF_RATE.NAME')"
                :desc="`${agentHandoff.handoffRatePercentage}`" :legend1="agentHandoff.data1.label"
                :legend1Value="`${agentHandoff.aiAgentHandoffCount}`" :legend2="agentHandoff.data2.label"
                :legend2Value="`${agentHandoff.handoffRate}`" />
            <div class="mt-4 h-72">
                <woot-loading-state v-if="accountReports.isFetching.ai_agent_message_send_count" class="text-xs"
                    :message="$t('REPORT.LOADING_CHART')" />
                <div v-else class="flex items-center justify-center h-72">
                    <BarChart2 v-if="agentHandoff.data1.data?.length" :data1="agentHandoff.data1"
