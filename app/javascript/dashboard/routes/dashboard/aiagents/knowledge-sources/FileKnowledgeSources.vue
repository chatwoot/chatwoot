<template>
    <div class="flex flex-row gap-4">
        <div class="flex-1 min-w-0 flex flex-col justify-stretch gap-4">
            ...
            <div class="border border-dashed rounded px-5 p-10 flex items-center justify-center cursor-pointer"
                @click="openPicker">
                <input id="inputfile" type="file" class="hidden" @change="v => addFile(v)" accept=".pdf">
                <span class="text-center">
                    <span>
                        Klik atau tarik dan lepas file di sini
                    </span>
                    <br>
                    <span>
                        Support file .pdf
            ...
            <div>
                <span>
                    File yg sudah ditambahkan:
                </span>
                <div class="py-2">
                    <div v-for="(item, index) in files" :key="index" class="flex flex-row gap-2 items-center">
...
const files = ref([])
const detectedCharacters = computed(() => files.value.reduce((p, i) => p + i.total_chars, 0))
