<template>
    <div class="flex flex-col justify-stretch gap-4">
        <div class="flex flex-row gap-2">
            <Button icon="i-lucide-plus" @click="() => addDoc()" :disabled="loadingAdd" />
            <div class="flex-1 min-w-0 flex flex-row gap-2">
                <div v-for="(item, index) in docs" :key="index"
                    ...
                    <Button icon="i-lucide-x" variant="link" @click="(e) => {
                        e.stopPropagation()
                        deleteDoc(index)
                    }" />
                </div>
...
async function updateKnowledge(data) {
    const summerNoteBtn = document.getElementsByClassName('save-btn-summernote')[0]
    try {
...
const selectedDoc = computed(() => docs.value.find((v, i) => i === selectedDocIndex.value))
watch(() => props.data, v => {
    const idAgent = v?.id
    if (idAgent) {
...
watch(selectedDoc, (newValue, prevValue) => {
...
onMounted(() => {
    var SaveButton = function (context) {
        ...
        var button = ui.button({
            contents: 'Simpan',
            tooltip: 'Simpan',
            className: 'save-btn-summernote bg-n-brand text-white hover:bg-n-brand hover:text-white font-[Inter]',
            click: function () {
                var markupStr = $('#summernote').summernote('code');
                updateKnowledge({
                    ...selectedDoc.value,
...
async function addDoc() {
    ...
    const maxTaNum = docs.value.reduce(function (prev, current) {
    ...
    try {
        await aiAgents.addKnowledgeText(props.data.id, {
            ...request,
        }).then(v => {
            docs.value.push({
                ...v.data,
            })
            selectedDocIndex.value = docs.value.findIndex((v) => v.tab === nextTabNum)
