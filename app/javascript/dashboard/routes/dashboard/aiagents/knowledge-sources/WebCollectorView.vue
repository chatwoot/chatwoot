<template>
    <woot-modal :show="showModel" :on-close="() => {
        showModel = false
    }" :size="'w-[1024px]'" modal>
        ...
        <div class="px-8 py-4 flex flex-col gap-2">
            <div>
                ...
                <div class="flex flex-row gap-2 items-center">
                    <Input class="flex-1" placeholder="Cari links" v-model="state.url" />
                    <Button size="sm" :is-loading="addingUrl" :disabled="addingUrl || v$.$invalid"
                        @click="() => addUrl()">Tambah</Button>
                </div>
            </div>
            <div class="flex flex-row gap-2 mt-3">
                <Button size="sm" variant="outline" @click="selectAll">Pilih Semua</Button>
                <Button size="sm" :is-loading="isTraining" :disabled="!hasSelectedItem || isTraining"
                    @click="() => train()">Train</Button>
            </div>
            <div class="flex flex-col gap-0">
                <div class="flex flex-row gap-4" v-for="(item, index) in mappedLinks" :key="index">
                    <CheckBox class="mt-5" :is-checked="item.isSelected.value" :value="item.isSelected.value" @update="(_, value) => {
                            ...
                            <div class="flex flex-row gap-2 items-center py-2 px-4">
                                ...
                                <div class="flex flex-row gap-3 items-center"
                                    ...
                                    <div class="rounded-lg bg-n-gray-4 text-sm px-2 py-1 cursor-pointer flex flex-row items-center gap-1"
                                        ...
                                        <span
                                            :class="item.toggleView.value ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'"
                                            class="size-5" aria-hidden="true" />
                                    </div>
                                </div>
                            </div>
                            <div class="flex flex-col px-4 gap-3 pb-4"
                                v-if="(item.children && item.children.length) && item.toggleView.value">
                                <span class="text-sm font-bold">
                                    Links
                                </span>
                                <div v-for="(item, index) in item.children" :key="index" class="flex flex-row gap-2">
                                    <div class="flex-1 flex flex-row gap-4 items-center">
                                        <CheckBox :is-checked="item.isSelected" :value="item.isSelected" @update="(_, value) => {
...
const mappedLinks = computed(() => {
    return links.value.map((e) => ({
        ...
        hasSelectedChild: e.children && e.children.length ? computed(() => {
            return e.children.some(t => t.isSelected)
        }) : ref(false),
        isSelected: e.children && e.children.length ? computed(() => {
...
async function addUrl() {
    ...
    try {
        ...
        if (activeTabIndex.value === 0) {
            const response = await aiAgents.collectKnowledgeLinksWebsite(props.idAgent, {
                url: stateUrl,
            })
...
const hasSelectedItem = computed(() => {
    return mappedLinks.value.some(t => t.children && t.children.length ? t.hasSelectedChild.value : t.isSelected.value)
})
function selectAll() {
    const isFirstSelected = mappedLinks.value.length ? mappedLinks.value[0].isSelected.value : false
