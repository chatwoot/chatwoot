...
const whatsappReferredProduct = computed(() => {
  const product =
    currentChat.value?.additional_attributes?.whatsapp_referred_product ?? null;
  return product ? useCamelCase(product) : null;
});
          ...
          <div v-else-if="element.name === 'whatsapp_referral'">
            ...
            >
              <WhatsAppReferral
                ...
                :referred-product="whatsappReferredProduct"
                class="px-2 py-4"
              />
            </AccordionItem>
          </div>
