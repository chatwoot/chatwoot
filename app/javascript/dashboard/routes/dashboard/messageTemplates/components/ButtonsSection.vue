...
const BUTTON_CATEGORY = {
  ...
  [BUTTON_TYPES.FLOW]: 'cta',
};

const buttonTypes = computed(() => [
  {
    label: t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.DROPDOWN.VISIT_WEBSITE'),
    icon: 'i-lucide-external-link',
    type: BUTTON_TYPES.URL,
  },
  {
    label: t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.DROPDOWN.CALL_PHONE'),
    icon: 'i-lucide-phone',
    type: BUTTON_TYPES.PHONE_NUMBER,
  },
  {
    label: t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.DROPDOWN.QUICK_REPLY'),
    icon: 'i-lucide-reply',
    type: BUTTON_TYPES.QUICK_REPLY,
  },
  {
    label: t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.DROPDOWN.COPY_CODE'),
    icon: 'i-lucide-copy',
    type: BUTTON_TYPES.COPY_CODE,
  },
  {
    label: t('WHATSAPP_FLOWS.TEMPLATE_BUILDER.FLOW_BUTTON'),
  ...
  <div class="space-y-6">
    ...
    <div v-if="buttonsData.length === 0">
      <h3 class="text-base font-medium text-n-slate-12 mb-2">
        {{ t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.BUTTONS_LABEL') }}
      </h3>
      ...
      <p class="text-sm text-n-slate-11 mt-2">
        {{ t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.DESCRIPTION') }}
      </p>
    </div>
    ...
    >
      <Button
        :label="t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.ADD_BUTTON')"
        icon="i-lucide-plus"
        :trailing-icon="
        ...
        <div class="grid gap-4 md:grid-cols-2 items-start">
          <div class="md:col-span-1 w-full">
            <label class="block text-xs font-medium text-n-slate-11 mb-1">
              {{ t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.BUTTON_TEXT') }}
            </label>
            <Input
              ...
              :placeholder="
                button.type === BUTTON_TYPES.QUICK_REPLY
                  ? t(
                      'MESSAGE_TEMPLATES.BUILDER.BUTTONS.PLACEHOLDERS.QUICK_REPLY'
                    )
                  : t(
                      'MESSAGE_TEMPLATES.BUILDER.BUTTONS.PLACEHOLDERS.BUTTON_TEXT'
                    )
              "
            ...
            >
              {{ t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.DROPDOWN.COPY_CODE') }}
            </div>
          </div>
          ...
          >
            <VariableSection
              ...
              :max-length="2000"
              :placeholder="
                t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.URL_PLACEHOLDER')
              "
              :label="t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.URL')"
          ...
          >
            <label class="block text-xs font-medium text-n-slate-11 mb-1">
              {{ t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.PHONE_NUMBER') }}
            </label>
            <Input
              :model-value="button.phone_number"
              :placeholder="
                t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.PLACEHOLDERS.PHONE_NUMBER')
              "
          ...
          >
            <label class="block text-xs font-medium text-n-slate-11 mb-1">
              {{ t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.EXAMPLE_CODE_LABEL') }}
            </label>
            <Input
              :model-value="button.example"
              :placeholder="
                t('MESSAGE_TEMPLATES.BUILDER.BUTTONS.EXAMPLE_CODE_PLACEHOLDER')
              "
          ...
          <!-- Flow Fields -->
          <template v-if="button.type === BUTTON_TYPES.FLOW">
            <div class="md:col-span-1 w-full">
              <label class="block text-xs font-medium text-n-slate-11 mb-1">
                {{ t('WHATSAPP_FLOWS.TEMPLATE_BUILDER.FLOW_ID') }}
              </label>
              <Input
                :model-value="button.flowId"
                :placeholder="
                  t('WHATSAPP_FLOWS.TEMPLATE_BUILDER.FLOW_ID_PLACEHOLDER')
                "
                @update:model-value="updateButton(index, 'flowId', $event)"
              />
            </div>
            <div class="md:col-span-1 w-full">
              <label class="block text-xs font-medium text-n-slate-11 mb-1">
                {{ t('WHATSAPP_FLOWS.TEMPLATE_BUILDER.FLOW_ACTION') }}
              </label>
              <select
                :value="button.flowAction"
                class="w-full px-3 py-2 text-sm border border-n-weak rounded-lg"
                @change="updateButton(index, 'flowAction', $event.target.value)"
              >
                <option value="navigate">
                  {{ t('WHATSAPP_FLOWS.TEMPLATE_BUILDER.ACTION_NAVIGATE') }}
                </option>
                <option value="data_exchange">
                  {{
                    t('WHATSAPP_FLOWS.TEMPLATE_BUILDER.ACTION_DATA_EXCHANGE')
                  }}
                </option>
              </select>
            </div>
            <div class="md:col-span-1 w-full">
              <label class="block text-xs font-medium text-n-slate-11 mb-1">
                {{ t('WHATSAPP_FLOWS.TEMPLATE_BUILDER.NAVIGATE_SCREEN') }}
              </label>
              <Input
                :model-value="button.navigateScreen"
                :placeholder="
                  t(
                    'WHATSAPP_FLOWS.TEMPLATE_BUILDER.NAVIGATE_SCREEN_PLACEHOLDER'
                  )
                "
                @update:model-value="
                  updateButton(index, 'navigateScreen', $event)
                "
              />
            </div>
          </template>
