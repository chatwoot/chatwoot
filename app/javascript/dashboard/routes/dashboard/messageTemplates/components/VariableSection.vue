...
const showNamedInput = ref(false);
...
const canAddVariable = computed(() => {
  return props.modelValue.examples.length < props.maxVariables;
});

const getErrorMessage = (errorKey, data = {}) => {
  if (errorKey === 'NAMED_IN_POSITIONAL') {
    return t('MESSAGE_TEMPLATES.BUILDER.VARIABLES.ERRORS.NAMED_IN_POSITIONAL');
  }
  if (errorKey === 'POSITIONAL_IN_NAMED') {
    return t('MESSAGE_TEMPLATES.BUILDER.VARIABLES.ERRORS.POSITIONAL_IN_NAMED');
  }
  if (errorKey === 'DUPLICATE_VARIABLES') {
    return t('MESSAGE_TEMPLATES.BUILDER.VARIABLES.ERRORS.DUPLICATE_VARIABLES');
  }
  if (errorKey === 'MAX_VARIABLES_EXCEEDED') {
    return t(
      'MESSAGE_TEMPLATES.BUILDER.VARIABLES.ERRORS.MAX_VARIABLES_EXCEEDED',
      data
    );
  }
  return '';
...
const addPositionalVariable = () => {
  ...
  const nextIndex = props.modelValue.examples.length + 1;
  const variable = `{{${nextIndex}}}`;
  const currentText = props.modelValue.text || '';
  const separator = currentText.length > 0 ? ' ' : '';
  updateText(`${currentText}${separator}${variable}`);
};

const addNamedVariable = () => {
  const name = namedVariableInput.value.trim().replace(/\s+/g, '_');
  if (!name || !canAddVariable.value) return;
  const variable = `{{${name}}}`;
  const currentText = props.modelValue.text || '';
  const separator = currentText.length > 0 ? ' ' : '';
  updateText(`${currentText}${separator}${variable}`);
  namedVariableInput.value = '';
  showNamedInput.value = false;
};

const handleAddVariable = () => {
  if (props.parameterType === 'positional') {
    addPositionalVariable();
  } else {
    showNamedInput.value = !showNamedInput.value;
  }
};
  ...
  <div class="space-y-4">
    ...
    <div>
      ...
      <div v-if="helpText" class="mt-1">
        <span class="text-xs text-n-slate-11">{{ helpText }}</span>
      </div>
      ...
      <div class="mt-2 flex items-center gap-2">
        <Button
          v-if="canAddVariable"
          :label="t('MESSAGE_TEMPLATES.BUILDER.VARIABLES.ADD_VARIABLE')"
          icon="i-lucide-plus"
          variant="ghost"
          color="blue"
          size="xs"
        ...
        />
        <!-- Named variable inline input -->
        <div
          v-if="showNamedInput && parameterType === 'named'"
        ...
        >
          <input
            ...
            :placeholder="
              t(
                'MESSAGE_TEMPLATES.BUILDER.VARIABLES.NAMED_VARIABLE_PLACEHOLDER'
              )
            "
            class="px-2 py-1 text-sm border rounded-lg border-n-strong bg-n-alpha-black2 text-n-slate-12 placeholder:text-n-slate-9 focus:outline-none focus:ring-2 focus:ring-n-brand w-40"
            @keyup.enter="addNamedVariable"
          />
          <Button
            :label="t('MESSAGE_TEMPLATES.BUILDER.VARIABLES.INSERT')"
            size="xs"
            variant="solid"
    ...
    <div v-if="hasVariables" class="space-y-3">
      <h5 class="text-sm font-medium text-n-slate-12 flex items-center gap-2">
        ...
        {{ t('MESSAGE_TEMPLATES.BUILDER.VARIABLES.TITLE') }}
      </h5>
        ...
        <div v-if="parameterType === 'positional'" class="space-y-2">
          ...
          >
            ...
            <Input
              :model-value="example"
              :placeholder="
                t('MESSAGE_TEMPLATES.BUILDER.VARIABLES.EXAMPLE_PLACEHOLDER', {
                  index: index + 1,
                })
              "
        ...
        <div v-if="parameterType === 'named'" class="space-y-2">
          ...
          >
            ...
            <Input
              ...
              :placeholder="
                t(
                  'MESSAGE_TEMPLATES.BUILDER.VARIABLES.NAMED_EXAMPLE_PLACEHOLDER',
