...
class ChatbotAPI extends ApiClient {
  ...
  async createChatbot(data) {
    ...
    async storeToDb(payload) {
        try {
            const response = await axios.post(`${this.baseUrl}/api/v1/widget/store-to-db`, payload, {
                headers: {
    ...
    async fetchChatbotsWithAccountId(accountId) {
        return await axios.get(`${this.baseUrl}/api/v1/widget/chatbot-with-account-id?account_id=${accountId}`);
    }

    async deleteChatbotWithChatbotId(chatbotId) {
        return await axios.delete(`${this.baseUrl}/api/v1/widget/chatbot-with-chatbot-id?chatbot_id=${chatbotId}`);
    }

    async editBotInfo(payload) {
        return await axios.put(`${this.baseUrl}/api/v1/widget/update-bot-info`, payload);
    }

    async toggleChatbotStatus (payload) {
        return await axios.post(`${this.baseUrl}/api/v1/widget/toggle-chatbot-status`, payload);
    }

    async fetchChatbotStatus(conversation_id) {
        return await axios.get(`${this.baseUrl}/api/v1/widget/chatbot-status?conversation_id=${conversation_id}`);
    }
}
