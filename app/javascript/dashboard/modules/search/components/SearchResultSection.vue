...
<template>
  <section class="mx-0 mb-3">
    ...
    >
      <h3 class="text-sm text-s-muted">{{ title }}</h3>
    </div>
    <slot />
    <woot-loading-state
      v-if="isFetching"
    ...
    <div
      ...
      class="flex items-start justify-center px-4 py-6 rounded-xl bg-s-subtle dark:bg-s-surface"
    >
      <Icon
        icon="i-lucide-info"
        class="text-s-muted size-4 flex-shrink-0 mt-[3px]"
      />
      <p class="mx-2 my-0 text-center text-s-muted">
