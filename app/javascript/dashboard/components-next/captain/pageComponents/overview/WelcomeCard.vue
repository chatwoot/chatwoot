...
// Markdown summary generated by the model from the assistant's live stats,
// queried server-side by the captain/assistants/:id/summary endpoint. Numbers
// are emphasised with **bold** so we can highlight them (see prose-strong
// styling below).
const welcomeMarkdown = ref('');
const isLoading = ref(false);

// Increments on every fetch so a slow response for a superseded
// range/assistant can't overwrite the latest request's state.
let fetchToken = 0;
