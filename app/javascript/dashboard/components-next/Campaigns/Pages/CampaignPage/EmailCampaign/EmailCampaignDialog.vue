...
const addCampaign = async campaignDetails => {
  try {
    const { data } = await store.dispatch('campaigns/create', campaignDetails);

    useTrack(CAMPAIGNS_EVENTS.CREATE_CAMPAIGN, {
      type: CAMPAIGN_TYPES.ONE_OFF,
    });

    // Cold outreach: fire the campaign immediately after creation.
    await store.dispatch('campaigns/trigger', data.id);
