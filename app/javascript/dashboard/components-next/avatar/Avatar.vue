...
const STATUS_CLASSES = computed(() => ({
  online: 'bg-s-success',
  busy: 'bg-s-warning',
  ...(props.hideOfflineStatus ? {} : { offline: 'bg-s-muted' }),
}));
  ...
  >
    ...
    <slot name="badge" :size="size">
      <div
        ...
        class="absolute z-20 border rounded-full border-s-border-subtle"
        :style="badgeStyles"
      ...
      <div
        ...
        :style="badgeStyles"
        class="absolute z-20 flex items-center justify-center rounded-full bg-s-surface border border-transparent flex-shrink-0"
      >
        <ChannelIcon :inbox="inbox" class="w-full h-full text-s-muted" />
      </div>
    </slot>
    ...
    <div
      ...
      class="absolute z-20 flex items-center justify-center invisible w-6 h-6 transition-all duration-300 ease-in-out opacity-0 cursor-pointer outline outline-1 outline-s-border -top-2 ltr:-right-2 rtl:-left-2 rounded-xl bg-s-subtle group-hover/avatar:visible group-hover/avatar:opacity-100"
      @click="handleDismiss"
    >
      <Icon icon="i-lucide-x" class="text-s-muted size-4" />
    </div>
    ...
    <span
      ...
      :class="[
        borderRadiusClass,
        {
          ...
          'bg-s-subtle dark:bg-s-subtle': showDefaultAvatar,
        },
      ]"
      :style="avatarStyles"
    >
      ...
      >
        <div
          class="absolute inset-0 z-10 flex items-center justify-center invisible w-full h-full transition-all duration-300 ease-in-out opacity-0 bg-s-primary/10 group-hover/avatar:visible group-hover/avatar:opacity-100"
