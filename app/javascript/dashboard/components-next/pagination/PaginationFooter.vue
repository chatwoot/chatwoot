...
<template>
  <div
    class="flex justify-between h-[3.375rem] w-full border-t border-s-border mx-auto bg-s-surface py-3 px-6 items-center before:absolute before:inset-x-0 before:-top-4 before:bg-gradient-to-t before:from-n-surface-1 before:from-0% before:to-transparent before:h-4 before:pointer-events-none"
  >
    <div class="flex items-center gap-3">
      <span class="min-w-0 text-body-main line-clamp-1 text-s-muted">
    ...
    <div class="flex items-center gap-2">
      ...
      <div class="inline-flex items-center gap-2 text-sm">
        <span
          class="px-3 tabular-nums py-0.5 font-420 bg-s-surface text-body-main text-s-primary rounded-md"
        >
          {{ formatFullNumber(currentPage) }}
        </span>
        <span class="truncate text-body-main text-s-muted">
          {{ pageInfo }}
        </span>
      </div>
      <Button
