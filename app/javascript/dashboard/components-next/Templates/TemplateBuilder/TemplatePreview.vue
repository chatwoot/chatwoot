...
<template>
  <div :class="colors.outerBg" class="rounded-xl p-4 min-h-[400px]">
    <!-- Phone frame mockup -->
    <div class="rounded-lg overflow-hidden shadow-xl" :style="{ backgroundColor: colors.phoneBg }">
      <!-- WhatsApp header -->
      <div class="px-4 py-3 flex items-center gap-3" :style="{ backgroundColor: colors.headerBg }">
        <div class="w-8 h-8 rounded-full flex items-center justify-center" :style="{ backgroundColor: colors.accentColor }">
      ...
      >
        ...
        <div v-else class="max-w-[85%]">
          <div class="rounded-lg overflow-hidden shadow-md" :style="{ backgroundColor: colors.bubbleBg }">
            <!-- Header -->
            <div v-if="headerFormat">
              <!-- Text Header -->
              <div v-if="headerFormat === 'TEXT' && (renderedHeaderText || headerText)" class="px-3 pt-2">
                <p class="text-sm font-semibold" :style="{ color: colors.textPrimary }">
                  {{ renderedHeaderText || headerText }}
                </p>
              </div>

              <!-- Image Header -->
              <div v-else-if="headerFormat === 'IMAGE'" :style="{ backgroundColor: colors.mediaBg }">
              ...
              <!-- Video Header -->
              <div v-else-if="headerFormat === 'VIDEO'" :style="{ backgroundColor: colors.mediaBg }">
                <div class="aspect-video flex items-center justify-center">
                  <div v-if="headerMediaUrl" class="text-center">
                    <div class="w-12 h-12 rounded-full flex items-center justify-center mb-2" :style="{ backgroundColor: colors.accentColor + '33' }">
                      <i class="i-lucide-play text-2xl" :style="{ color: colors.accentColor }" />
                    </div>
                  </div>
              ...
              <!-- Document Header -->
              <div v-else-if="headerFormat === 'DOCUMENT'" class="p-3" :style="{ backgroundColor: colors.mediaBg }">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded flex items-center justify-center" :style="{ backgroundColor: colors.accentColor + '33' }">
              ...
              <!-- Location Header -->
              <div v-else-if="headerFormat === 'LOCATION'" :style="{ backgroundColor: colors.mediaBg }">
                <div class="aspect-video flex items-center justify-center">
                  <div class="text-center">
            ...
            <!-- Body -->
            <div class="px-3 py-2">
              <p class="text-sm whitespace-pre-wrap leading-relaxed" :style="{ color: colors.textPrimary }">{{ renderedBodyText || 'Message body...' }}</p>
            </div>

            <!-- Footer -->
            <div v-if="footerText" class="px-3 pb-1">
              <p class="text-xs" :style="{ color: colors.textSecondary }">{{ footerText }}</p>
            </div>
          ...
          <div v-if="validButtons.length > 0" class="mt-1 space-y-1">
            <button
              ...
              :style="{ backgroundColor: colors.buttonBg, color: colors.linkColor }"
            >
