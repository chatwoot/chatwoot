...
<template>
  <div class="flex-1 flex flex-col gap-6 px-2">
    <div class="flex flex-col space-y-4 py-4">
      <Icon icon="i-woot-captain" class="text-s-disabled text-4xl" />
      <div class="space-y-1">
        <h3 class="text-base font-medium text-s-primary leading-8">
          {{ $t('CAPTAIN.COPILOT.PANEL_TITLE') }}
        </h3>
        <p class="text-sm text-s-muted leading-6">
    ...
    <div v-if="!hasAssistants" class="w-full space-y-2">
      ...
      <router-link
        ...
        class="text-s-muted underline hover:text-s-primary"
      >
        {{ $t('CAPTAIN.ASSISTANTS.ADD_NEW') }}
      </router-link>
    </div>
    <div v-else class="w-full space-y-2">
      <span class="text-xs text-s-muted block">
      ...
      <div class="space-y-1">
        <button
          ...
          :key="prompt.label"
          class="w-full px-3 py-2 rounded-md border border-s-border bg-s-subtle text-s-muted flex items-center justify-between hover:bg-s-subtle transition-colors"
