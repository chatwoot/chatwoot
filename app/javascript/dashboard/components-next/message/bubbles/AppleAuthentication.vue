  ...
  <div class="apple-authentication-bubble">
    <div class="auth-container">
      <div class="auth-header">
        <div class="auth-icon">
          <i class="i-ph-key-duotone text-2xl"></i>
        </div>
        <div class="auth-content">
          <h3 class="auth-title">{{ $t('APPLE_MESSAGES.AUTHENTICATION.TITLE') }}</h3>
          <p class="auth-description">
            {{ $t('APPLE_MESSAGES.AUTHENTICATION.DESCRIPTION', { provider: providerName }) }}
          </p>
        </div>
      </div>
      ...
      <div class="auth-actions">
        ...
        >
          ...
          {{ $t('APPLE_MESSAGES.AUTHENTICATION.SIGN_IN', { provider: providerName }) }}
        </button>
      ...
      <div v-if="authResult" class="auth-success">
        ...
        {{ $t('APPLE_MESSAGES.AUTHENTICATION.SUCCESS', { name: authResult.user.name }) }}
      </div>
    </div>
  </div>
</template>
  ...
  setup(props, { emit }) {
    ...
    const checkAuthenticationResult = async () => {
      ...
      const pollResult = setInterval(async () => {
        ...
        try {
          const response = await fetch(`/apple_messages_for_business/${props.mspId}/oauth/status?state=${props.state}`);
    ...
    const providerName = computed(() => {
      ...
      return providers[props.oauth2Provider.toLowerCase()] || props.oauth2Provider;
    });
    ...
    const startAuthentication = async () => {
      try {
        ...
        const response = await fetch(`/apple_messages_for_business/${props.mspId}/oauth/landing/create`, {
          method: 'POST',
          headers: {
