...
const getMessageClasses = {
  emphasis: 'text-sm font-medium text-s-muted',
  emphasisUnread: 'text-sm font-medium text-s-primary',
  normal: 'text-sm font-normal text-s-muted',
  normalUnread: 'text-sm text-s-primary',
};
...
const formattedMessage = computed(() => {
  ...
  return isUnread.value
    ? `<span class="inline-flex flex-shrink-0 w-2 h-2 mb-px rounded-full bg-s-brand-cta ltr:mr-1 rtl:ml-1"></span> ${messageContent}`
    : messageContent;
});

const notificationDetails = computed(() => {
  const type = props.inboxItem?.notificationType?.toUpperCase() || '';
  const [icon = '', color = 'text-s-brand-text'] =
    ...
    <div class="flex items-center justify-between h-6 gap-2">
      <div class="flex items-center flex-1 min-w-0 gap-1">
        ...
        >
          <Icon
            ...
            :class="!isUnread ? 'text-s-muted' : 'text-s-brand-text'"
          />
          <span
        ...
        >
          <Icon
            ...
            :class="isUnread ? notificationDetails.color : 'text-s-muted'"
            class="flex-shrink-0 size-4"
          />
          <span
            class="text-xs font-medium truncate"
      ...
      <div class="flex items-center flex-shrink-0 gap-2">
        <SLACardLabel
          ...
          class="[&>span]:text-xs"
          :class="
            !isUnread && '[&>span]:text-s-muted [&>div>svg]:fill-s-muted'
          "
        />
        <div v-if="hasSlaThreshold" class="w-px h-3 rounded-sm bg-s-subtle" />
        <CardPriorityIcon
