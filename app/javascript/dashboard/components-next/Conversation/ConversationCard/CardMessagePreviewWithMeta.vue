...
const assignee = computed(() => {
  const { meta = {} } = props.conversation;
  return {
    name: meta?.assignee?.name || meta?.assignee?.available_name,
    thumbnail: meta?.assignee?.thumbnail,
    status: meta?.assignee?.availability_status,
  };
});
