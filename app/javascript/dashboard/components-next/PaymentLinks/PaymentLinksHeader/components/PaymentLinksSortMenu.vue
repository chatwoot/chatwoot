...
const sortMenus = [
  {
    label: t('PAYMENT_LINKS_LAYOUT.HEADER.ACTIONS.SORT_BY.OPTIONS.CREATED_AT'),
    value: 'created_at',
  },
  {
    label: t('PAYMENT_LINKS_LAYOUT.HEADER.ACTIONS.SORT_BY.OPTIONS.AMOUNT'),
    value: 'amount',
  },
  {
    label: t(
...
const orderingMenus = [
  {
    label: t('PAYMENT_LINKS_LAYOUT.HEADER.ACTIONS.ORDER.OPTIONS.ASCENDING'),
    value: '',
  },
  {
    label: t('PAYMENT_LINKS_LAYOUT.HEADER.ACTIONS.ORDER.OPTIONS.DESCENDING'),
    value: '-',
  },
];
...
const activeSortLabel = computed(() => {
  ...
  return (
    selectedMenu?.label ||
    t('PAYMENT_LINKS_LAYOUT.HEADER.ACTIONS.SORT_BY.LABEL')
  );
});
...
const activeOrderingLabel = computed(() => {
  ...
  return (
    selectedMenu?.label || t('PAYMENT_LINKS_LAYOUT.HEADER.ACTIONS.ORDER.LABEL')
  );
});
...
<template>
  <div class="relative">
    ...
    >
      ...
      <div class="flex items-center justify-between gap-2">
        <span class="text-sm text-n-slate-12">
          {{ t('PAYMENT_LINKS_LAYOUT.HEADER.ACTIONS.ORDER.LABEL') }}
        </span>
        <SelectMenu
