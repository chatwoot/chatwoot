...
describe('#actions', () => {
  ...
  describe('#create', () => {
    it('sends correct actions if API is success', async () => {
      axios.post.mockResolvedValue({ data: automationsList.payload[0] });
      await actions.create({ commit }, automationsList.payload[0]);
      expect(commit.mock.calls).toEqual([
        [types.default.SET_AUTOMATION_UI_FLAG, { isCreating: true }],
        [types.default.ADD_AUTOMATION, automationsList.payload[0]],
  ...
  describe('#update', () => {
    it('sends correct actions if API is success', async () => {
      axios.patch.mockResolvedValue({ data: automationsList.payload[0] });

      await actions.update({ commit }, automationsList.payload[0]);
      expect(commit.mock.calls).toEqual([
        [types.default.SET_AUTOMATION_UI_FLAG, { isUpdating: true }],
        [types.default.EDIT_AUTOMATION, automationsList.payload[0]],
    ...
    it('sends correct actions if API is error', async () => {
      ...
      await expect(
        actions.update({ commit }, automationsList.payload[0])
  ...
  describe('#delete', () => {
    it('sends correct actions if API is success', async () => {
      axios.delete.mockResolvedValue({ data: automationsList.payload[0] });
      await actions.delete({ commit }, automationsList.payload[0].id);
      expect(commit.mock.calls).toEqual([
        [types.default.SET_AUTOMATION_UI_FLAG, { isDeleting: true }],
        [types.default.DELETE_AUTOMATION, automationsList.payload[0].id],
    ...
    it('sends correct actions if API is error', async () => {
      ...
      await expect(
        actions.delete({ commit }, automationsList.payload[0].id)
      ).rejects.toThrow(Error);
      expect(commit.mock.calls).toEqual([
        [types.default.SET_AUTOMATION_UI_FLAG, { isDeleting: true }],
        [types.default.SET_AUTOMATION_UI_FLAG, { isDeleting: false }],
  ...
  describe('#clone', () => {
    it('clones the automation', async () => {
      axios.post.mockResolvedValue({ data: automationsList.payload[0] });
      await actions.clone({ commit }, automationsList.payload[0]);
      expect(commit.mock.calls).toEqual([
