...
describe('#actions', () => {
  ...
  describe('#setInboxPolicy', () => {
    it('returns response data if API is success', async () => {
      const responseData = { success: true, policy_id: 2 };
      axios.post.mockResolvedValue({ data: responseData });

      const result = await actions.setInboxPolicy({ inboxId: 1, policyId: 2 });
    ...
    it('throws error if API fails', async () => {
      ...
      await expect(
        actions.setInboxPolicy({ inboxId: 1, policyId: 2 })
      ).rejects.toThrow(Error);
    });
  });
  ...
  describe('#getInboxPolicy', () => {
    it('returns response data if API is success', async () => {
      ...
      const result = await actions.getInboxPolicy(1);
    ...
    it('throws error if API fails', async () => {
      ...
      await expect(actions.getInboxPolicy(999)).rejects.toThrow(Error);
    });
  });

  describe('#removeInboxPolicy', () => {
    it('completes successfully if API is success', async () => {
      axios.delete.mockResolvedValue({});

      await expect(actions.removeInboxPolicy(1)).resolves.toBeUndefined();
    });

    it('throws error if API fails', async () => {
      axios.delete.mockRejectedValue(new Error('Not found'));

      await expect(actions.removeInboxPolicy(999)).rejects.toThrow(Error);
    });
  });
});
