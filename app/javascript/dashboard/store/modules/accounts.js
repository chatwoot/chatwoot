...
export const actions = {
  ...
  // Stubs for actions that previously hit EnterpriseAccountAPI (CEEL).
  // Removed in PainelChat fork. UI components still dispatch these for now;
  // we let them resolve silently so the dashboard does not crash. Replace
  // with native PainelChat implementations as needed.
  toggleDeletion: async () => Promise.resolve(),
  checkout: async () => Promise.resolve(),
  subscription: async () => Promise.resolve(),
  limits: async () => Promise.resolve(),
