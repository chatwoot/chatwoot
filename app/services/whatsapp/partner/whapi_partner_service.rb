...
module Whatsapp
  module Partner
    class WhapiPartnerService
      ...
      base_uri ENV['WHAPI_PARTNER_BASE_URL']
      ...
      def initialize(partner_token: ENV['WHAPI_PARTNER_TOKEN'], api_base_url: ENV['WHAPI_API_BASE_URL'])
        @partner_headers = {
      ...
      def create_channel(name:, project_id:)
        ...
        unless channel_id.present? && channel_token.present?
          raise(StandardError, 'WHAPI Partner create_channel response missing id or token')
        end
      ...
      def update_channel_webhook(channel_token:, webhook_url:, retries: 3)
        ...
        payload = {
          ...
          media: {
            auto_download: ['image', 'audio', 'voice', 'video', 'document', 'sticker']
          }
        }
      ...
      def generate_qr_code_base64(channel_token:)
        ...
        begin
          ...
          if response.body.present? && response.body.match?(/^[A-Za-z0-9+\/=]+$/)
