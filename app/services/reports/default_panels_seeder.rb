# Seeds the three default saved report panels for a new account.
# Idempotent by panel name — safe if migration already created them.
class Reports::DefaultPanelsSeeder
  DEFAULT_NAMES = [
  ...
  def perform
    creator = @account.administrators.order(:id).first || @account.users.order(:id).first
    ...
    default_panels.each do |attrs|
      ...
      @account.saved_report_panels.create!(
        attrs.merge(created_by: creator, filters: [], business_hours: false)
      )
    end
  end
