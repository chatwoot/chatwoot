class X::IncomingMessageService
  ...
  def find_existing_conversation
    return contact_inbox.conversations.where("additional_attributes ->> 'type' = 'direct_message'").first unless tweet_message?

    Conversation.where(inbox_id: channel.inbox.id)
                .where("additional_attributes ->> 'tweet_id' = ?", parent_tweet_id)
                .first || channel.inbox.messages.find_by(source_id: parent_tweet_id)&.conversation
  end
  ...
  def conversation_additional_attributes
    tweet_message? ? { type: 'tweet', tweet_id: parent_tweet_id } : { type: 'direct_message' }
  end
  ...
  def update_conversation_tweet_id
    conversation.update!(additional_attributes: (conversation.additional_attributes || {}).merge('tweet_id' => parent_tweet_id))
  end
  ...
  def create_dm_attachment(msg, attachment)
    ...
    att = msg.attachments.new(account_id: msg.account_id, file_type: file_type_from_content_type(media[:content_type]), external_url: media[:url])
    attach_file(att, media[:url])
  end
  ...
  def create_tweet_attachments(msg)
    @tweet_data[:extended_entities][:media]&.each do |media|
      ...
      att = msg.attachments.new(account_id: msg.account_id, file_type: file_type, external_url: url)
      attach_file(att, url)
    end
  end
  ...
  def media_url_for_tweet(media)
    return media[:media_url_https] unless media[:type].in?(%w[video animated_gif])

    best_video_variant(media) || media[:media_url_https]
  end

  def best_video_variant(media)
    media.dig(:video_info, :variants)
  ...
  def message_content
    ...
    clean_tweet_text if tweet_message?
  end
  ...
  def sender_id
    direct_message? ? @dm_event.dig(:message_create, :sender_id) : @tweet_data&.dig(:user, :id_str)
  end

  def message_source_id
    direct_message? ? @dm_event[:id] : @tweet_data&.[](:id_str)
  end
