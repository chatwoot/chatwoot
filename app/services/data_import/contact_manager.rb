class DataImport::ContactManager
  ...
  def find_or_initialize_contact(params)
    ...
    contact_params = params.slice(:id, :email, :identifier, :phone_number)
  ...
  def find_existing_contact(params)
    contact = find_contact_by_id(params)
  ...
  def find_contact_by_id(params)
    return unless params[:id]

    @account.contacts.find_by(id: params[:id])
  end
  ...
  def update_contact_attributes(params, contact)
    ...
    contact.assign_attributes(custom_attributes: contact.custom_attributes.merge(params.except(:id, :identifier, :email, :name, :phone_number)))
  end
end
