...
class Aloo::ProactiveOutreachService
  def initialize(conversation, event_context)
    @conversation = conversation
    @event_context = event_context.with_indifferent_access
  ...
  def call
    ...
    with_context do
      with_typing_indicator do
        ...
        send_outreach(result) if result.success? && result.content.present?
      end
    end
  end
  ...
  def processable?
    @assistant&.active? && !human_assigned?
  end
  ...
  def with_context
    ...
    Aloo::Current.inbox = @conversation.inbox
  ...
  def send_whatsapp_typing_indicator
    ...
    Rails.logger.info "[ALOO] Sending WhatsApp typing indicator for proactive outreach to #{phone_number}"
  ...
  def generate_outreach_message
    ProactiveOutreachAgent.call(event_context: @event_context)
  end

  def send_outreach(result)
    ...
    Messages::MessageBuilder.new(
      ...
      @conversation,
      {
        ...
        content_attributes: {
          ...
          'aloo_proactive_outreach' => true,
          'moengage_event' => @event_context[:event_name],
