class MessageTemplates::Template::AutoResolve
  pattr_initialize [:conversation!, :message_text, { message_type: :template }]

  def perform
    return if auto_resolve_message.blank?
  ...
  def auto_resolve_message_params
    {
      ...
      content: auto_resolve_message
    }
  end

  def auto_resolve_message
    message_text.presence || account.auto_resolve_message
  end
end
