# Service to generate Facebook Conversions API access token for Instagram
# Instagram uses the same Facebook Conversions API infrastructure
class Instagram::GenerateTokenService
  ...
  # Generate a Conversions API access token for Instagram
  def generate_conversions_api_token
    ...
    # Try each Instagram channel until we find one that works
    instagram_channels.each do |channel|
      result = try_generate_token_for_channel(channel)
      return result if result[:success]
    end

    { success: false, error: 'Could not generate token through any Instagram channel' }
  end
  ...
  def try_generate_token_for_channel(channel)
    ...
    begin
      ...
      unless verify_pixel_access_through_channel(channel)
        return { success: false, error: 'Pixel not accessible through this Instagram channel' }
      end
      ...
      Rails.logger.error("Instagram authentication error: #{e.message}")
      { success: false, error: 'Instagram authentication failed. Please reconnect your account.' }
    rescue StandardError => e
      Rails.logger.error("Error generating token for Instagram channel #{channel.id}: #{e.message}")
  ...
  def get_business_id_for_channel(channel)
    ...
    begin
      # Get businesses associated with the Facebook page
      businesses = facebook_api.get_connections(channel.page_id, 'businesses', fields: 'id,name')
  ...
  def generate_system_user_token(channel, business_id)
    ...
    begin
      # Create a system user for Instagram Conversions API
      system_user_params = {
        name: "Mooly Instagram Conversions API - #{Time.current.strftime('%Y%m%d_%H%M%S')}",
        role: 'ADMIN'
      }
      ...
      token_params = {
        scope: 'ads_management,business_management,instagram_basic,instagram_manage_messages'
      }
      ...
      if long_lived_token
        {
          ...
          note: 'Using extended user access token for Instagram. For production, consider using system user tokens.'
        }
      else
        { success: false, error: 'Could not generate any suitable access token for Instagram' }
      end
    rescue StandardError => e
      Rails.logger.error("Error generating extended user token for Instagram: #{e.message}")
      { success: false, error: 'Failed to generate access token. Please check your Instagram app configuration.' }
    end
  end
end
