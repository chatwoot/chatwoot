class MobilePush::DispatchService
  pattr_initialize [:message!]

  def perform
    return unless message.outgoing? && !message.private? && message.inbox.web_widget?

    devices = MobilePushDevice.where(contact_inbox_id: message.conversation.contact_inbox_id, contact_id: message.conversation.contact_id,
                                     invalidated_at: nil)
    devices.find_each do |device|
      delivery = device.mobile_push_deliveries.create_or_find_by!(message: message)
      MobilePush::DeliveryJob.perform_later(delivery.id) if delivery.status == 'pending'
    end
  end
end
