class Integrations::Stripe::Oauth
  ...
  def self.configured?
    GlobalConfig.get_value('STRIPE_APP_AUTHORIZE_URL').present? &&
      GlobalConfig.get_value('STRIPE_APP_SECRET_KEY').present? && Chatwoot.encryption_configured?
  end
  ...
  def self.authorize_url(account:, user:)
    ...
    raise ArgumentError, 'Invalid Stripe authorization URL' unless uri.scheme == 'https' && uri.host == 'marketplace.stripe.com'
