class Integrations::CustomApi::CreateOrUpdateContactService
  ...
  def create_or_update_contact
    contact_corrupted = find_contact_corrupted(first_contact[:contact_data])
    ...
    unless contact_corrupted.nil?
      ...
      return create_corrupted_contact(
        find_corrupted_data(contact_corrupted[:contact_data]), first_contact[:contact_data]
      )
    end
    create_contact
  end
  ...
  def create_corrupted_contact(corrupted_values, contact_finded)
    contact_attributes = {
      account_id: @custom_api['account_id'],
      name: "#{@account_data['firstName']} #{@account_data['lastName']}",
      custom_attributes: contact_custom_attributes(contact_finded),
  ...
  def find_contact_corrupted(contact_value) # rubocop:disable Metrics/AbcSize,Metrics/PerceivedComplexity,Metrics/CyclomaticComplexity
