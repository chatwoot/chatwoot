class SearchService
  ...
  def filter_conversations
    @conversations = current_account.conversations
                                    ...
                                    .where(
                                      "unaccent(cast(conversations.display_id as text)) ILIKE unaccent(:search) OR
                                       unaccent(contacts.name) ILIKE unaccent(:search) OR
                                       unaccent(contacts.email) ILIKE unaccent(:search) OR
                                       unaccent(contacts.phone_number) ILIKE unaccent(:search) OR
                                       unaccent(contacts.identifier) ILIKE unaccent(:search)",
                                      search: "%#{search_query}%"
                                    )
  ...
  def filter_contacts
    @contacts = current_account.contacts.where(
      "unaccent(name) ILIKE unaccent(:search) OR
       ...
       unaccent(phone_number) ILIKE unaccent(:search) OR
       unaccent(identifier) ILIKE unaccent(:search)", search: "%#{search_query}%"
