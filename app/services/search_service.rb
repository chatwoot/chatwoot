class SearchService
  ...
  def filter_conversations
    ...
    if search_query.match?(/^#(\d+)$/)
      ...
      conversations_query = base_query.where('conversations.display_id = ?', conversation_id)
    # If search query is numeric only (without #), do exact match on display_id
    elsif search_query.match?(/^\d+$/)
      conversations_query = base_query.joins('INNER JOIN contacts ON conversations.contact_id = contacts.id')
