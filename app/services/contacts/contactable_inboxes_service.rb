class Contacts::ContactableInboxesService
  ...
  def xmpp_contactable_inbox(inbox)
    return if @contact.custom_attributes["jid"].blank?

    { source_id: @contact.custom_attributes["jid"], inbox: inbox }
  end
end
