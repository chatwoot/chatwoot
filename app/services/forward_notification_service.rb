class ForwardNotificationService
  ...
  def send_whatsapp_notifications(target_chats)
    # Get the WhatsApp channel directly from the account
    whatsapp_channel = @account.whatsapp_channels.first
    
    if whatsapp_channel.nil?
      Rails.logger.warn "No WhatsApp channels found for account #{@account.id}"
      return
    end
