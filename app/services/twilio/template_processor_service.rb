class Twilio::TemplateProcessorService
  ...
  def find_template
    channel.content_templates['templates']&.find do |template|
  ...
  def build_content_variables(template)
    ...
    when 'text'
      convert_text_template(template_params)
    when 'media'
      convert_media_template(template_params)
    when 'quick_reply'
      convert_text_template(template_params) # Quick reply templates use body variables
    else
      {}
    end
  end
