class Channels::RefreshMsOauthTokenJob < ApplicationJob
  ...
  def perform
    Channel::Email.all.each do |channel|
      ...
      refresh_tokens(channel, ms_oauth_token_hash)
    end
  end
  ...
  def access_token(channel, ms_oauth_token_hash)
    expiry = Time.at(ms_oauth_token_hash[:expires_at] - 300)
  ...
  def update_channel_ms_oauth_tokens(channel, new_tokens)
    ...
    channel.ms_oauth_token_hash = { access_token: new_tokens.delete(:access_token), refresh_token: new_tokens.delete(:refresh_token), expires_at:  new_tokens.delete(:expires_at) }
    channel.save!
  end
end
