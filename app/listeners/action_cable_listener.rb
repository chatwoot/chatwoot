class ActionCableListener < BaseListener
  ...
  def message_created(event)
    ...
    tokens = user_tokens(account, conversation.inbox.agents) + contact_tokens(conversation.contact_inbox, message)
  ...
  def first_reply_created(event)
    message, account = extract_message_and_account(event)
    conversation = message.conversation
    tokens = user_tokens(account, conversation.inbox.agents)
  ...
  def conversation_created(event)
    ...
    tokens = user_tokens(account, conversation.inbox.agents) + contact_inbox_tokens(conversation.contact_inbox)
  ...
  def conversation_agent_read(event)
    ...
    broadcast(account, tokens, CONVERSATION_AGENT_READ, conversation.push_event_data)
  end
  ...
  def typing_event_listener_tokens(account, conversation, user)
    ...
    (user_tokens(account, conversation.inbox.agents) + [conversation.contact_inbox.pubsub_token]) - [current_user_token]
  end
