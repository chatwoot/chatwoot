class CsatSurveys::ResponseBuilder
  ...
  def process_csat_response(conversation, rating, feedback_message)
    csat_survey_response = message.csat_survey_response || CsatSurveyResponse.new(
      ...
      contact_id: conversation.contact_id, assigned_agent: resolving_agent
    )
  ...
  def resolving_agent
    # Use the CSAT message sender (agent who resolved the conversation)
    # Fall back to conversation assignee for backward compatibility with existing CSATs
    message.sender_type == 'User' ? message.sender : message.conversation.assignee
  end
end
