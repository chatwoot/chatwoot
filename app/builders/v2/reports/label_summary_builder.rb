class V2::Reports::LabelSummaryBuilder < V2::Reports::BaseSummaryBuilder
  ...
  def collect_report_data
...

    {
      ...
      first_response_event_counts: fetch_event_counts(conversation_filter, 'first_response'),
      reply_event_counts: fetch_event_counts(conversation_filter, 'reply_time')
    }
  end
  ...
  def aggregate_time_metrics(label_titles, report_data)
    {
      ...
      avg_first_response_time: calculate_weighted_average(
        label_titles, report_data[:first_response_metrics], report_data[:first_response_event_counts]
      ),
      avg_reply_time: calculate_weighted_average(
        label_titles, report_data[:reply_metrics], report_data[:reply_event_counts]
      )
    }
  end
  ...
  def fetch_event_counts(conversation_filter, event_name)
    ReportingEvent
      .joins(conversation: { taggings: :tag })
      .where(
        conversations: conversation_filter,
