...
class Webhooks::CallKnowlarityController < ActionController::API # rubocop:disable Metrics/ClassLength
  ...
  def handle_incoming_call(account, contact, parsed_body) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
    ...
    is_missed_call = parsed_body['call_type'] == 'incoming' && parsed_body['destination'] == 'Agent Missed' && parsed_body['agent_missed_number'].present? # rubocop:disable Layout/LineLength
  ...
  def handle_incoming_callback(conversation, parsed_body, account) # rubocop:disable Metrics/AbcSize
    ...
    if agent_phone.present?
      agent = account.users.find_by(
        "custom_attributes->>'phone_number' LIKE ?",
        "%#{agent_phone.gsub(/^((\+91)|0)+/, '')}%"
      )
    ...
    start_time = Time.parse(parsed_body['start_time']).in_time_zone('Asia/Kolkata').utc
    ...
    call_log_message = get_call_log_string_knowlarity(parsed_body)
  ...
  def find_account(parsed_body)
    did_number = parsed_body['dispnumber']
    Account.where("custom_attributes->>'call_config' IS NOT NULL")
           .find_by("custom_attributes->'call_config'->'externalProviderConfig'->>'k_number' = ?", did_number)
  ...
  def find_contact_or_create_contact(account, parsed_body, is_inbound) # rubocop:disable Metrics/MethodLength,Metrics/CyclomaticComplexity
    Rails.logger.info("parsed_bodyData #{parsed_body.inspect}")
    client_no = case parsed_body['call_type']
                when 'incoming'
                  parsed_body['caller_id']
                when 'outgoing'
                  if parsed_body['destination'] == 'Agent Missed'
                    parsed_body['caller_id']
                  elsif parsed_body['destination'] == 'Customer Missed'
                    parsed_body['customer_missed_number']
                  else
                    parsed_body['destination']
                  end
                end

    return nil if client_no.blank?
  ...
  def private_message_params(content, conversation, parsed_body)
    received_at = parsed_body['start_time'].presence || parsed_body['end_time']
  ...
  def send_call_log_to_bspd(parsed_body, conversation, account)
    ...
    call_report = build_call_report_knowlarity(parsed_body, conversation, account)
