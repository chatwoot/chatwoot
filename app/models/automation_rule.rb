...
class AutomationRule < ApplicationRecord
  ...
  def conditions_attributes
    %w[content email country_code status platform payment_status order_number message_type browser_language assignee_id team_id referer city company inbox_id
       mail_subject phone_number priority conversation_language contact_label conversation_label custom_status none]
  end

  def actions_attributes
    %w[send_message add_label remove_label add_contact_label remove_contact_label send_email_to_team assign_team assign_agent send_webhook_event mute_conversation
