class ContentAttributeValidator < ActiveModel::Validator
  ...
  ALLOWED_APPLE_LIST_PICKER_KEYS = [:sections, :images, :received_title, :received_subtitle, :received_image_identifier, :received_style, :reply_title, :reply_subtitle, :reply_image_title, :reply_image_subtitle, :reply_secondary_subtitle, :reply_tertiary_subtitle, :reply_image_identifier, :reply_style].freeze
  ...
  ALLOWED_APPLE_TIME_PICKER_KEYS = [:event, :timezone_offset, :timeslots, :received_title, :received_subtitle, :received_image_identifier, :received_style, :reply_title, :reply_subtitle, :reply_image_title, :reply_image_subtitle, :reply_secondary_subtitle, :reply_tertiary_subtitle, :reply_image_identifier, :reply_style].freeze
  ...
  ALLOWED_APPLE_FORM_KEYS = [:fields, :submit_url, :method, :validation_rules].freeze
  ALLOWED_APPLE_CUSTOM_APP_KEYS = [:app_id, :app_name, :bid, :url, :use_live_layout].freeze
  
  # Apple MSP style values
  APPLE_STYLE_VALUES = ['icon', 'small', 'large'].freeze
  ...
  def auto_generate_apple_identifiers!(record)
    ...
    when 'apple_list_picker'
      ...
      sections.each do |section|
        ...
        if section.key?('multiple_selection')
          section['multipleSelection'] = section.delete('multiple_selection')
        end
        ...
        items.each do |item|
          ...
          if item.key?('image_identifier')
            item['imageIdentifier'] = item.delete('image_identifier')
          end
  ...
  def validate_apple_list_picker!(record)
    ...
    sections.each_with_index do |section, index|
      ...
      items.each_with_index do |item, item_index|
        ...
        record.errors.add(:content_attributes, "section #{index} item #{item_index} contains invalid keys: #{invalid_item_keys}") if invalid_item_keys.present?
        
        # Validate style value if present
        if item['style'] && !APPLE_STYLE_VALUES.include?(item['style'])
          record.errors.add(:content_attributes, "section #{index} item #{item_index} has invalid style value: #{item['style']}. Must be one of: #{APPLE_STYLE_VALUES.join(', ')}")
        end
      end
    end
    ...
    ['received_style', 'reply_style'].each do |style_field|
  ...
  def validate_apple_rich_link!(record)
    ...
    if content_attrs['video_url'].present?
      ...
      begin
        ...
        record.errors.add(:content_attributes, 'video_url must be a valid HTTP/HTTPS URL') unless video_uri.is_a?(URI::HTTP) || video_uri.is_a?(URI::HTTPS)
      rescue URI::InvalidURIError
      ...
      unless ['video/mp4', 'video/mpeg', 'video/quicktime'].include?(video_mime_type)
  ...
  def validate_apple_authentication!(record)
    ...
    record.errors.add(:content_attributes, 'response_encryption_key is required for apple_authentication') if content_attrs['response_encryption_key'].blank?
  end
