...
class Label < ApplicationRecord
  ...
  after_save :recalculate_descendants_depth, if: :saved_change_to_parent_id?
  ...
  def subtree_max_relative_depth
    return 0 if children.empty?

    children.includes(:children).map { |child| 1 + child.subtree_max_relative_depth }.max
  end
  ...
  def recalculate_descendants_depth
    children.find_each do |child|
      child.update(depth: depth + 1)
      child.send(:recalculate_descendants_depth)
    end
  end
  ...
  def prevent_deep_nesting
    ...
    max_descendant_depth = persisted? ? subtree_max_relative_depth : 0
    deepest = calculated_depth + max_descendant_depth

    return unless deepest > max_depth
