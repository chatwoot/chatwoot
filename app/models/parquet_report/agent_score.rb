...
class ParquetReport::AgentScore < ParquetReport
  def process!
    ...
    url = Digitaltolk::AgentScoresParquetService.new(@smart_actions, file_name, self).perform
    complete_and_save_url!(url)
  end

  def create_empty_file_url
    url = Digitaltolk::AgentScoresParquetService.new([], file_name, self).perform
  ...
  def load_smart_actions
    ...
    @smart_actions = account.smart_actions.outgoing_message_score.filter_by_created_at(range)
  end
end