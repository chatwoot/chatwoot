...
class Message < ApplicationRecord
  ...
  store :content_attributes, accessors: [:submitted_email, :items, :submitted_values, :email, :in_reply_to, :comment_id, :deleted,
                                         ...
                                         :reply_to_comment_id, :should_prompt_resolution, :is_dm_conversation_created, :user_phone_number, :previous_selected_replies, :user_order_id, :selected_reply, :product_id, :product_id_for_more_info, :product_page, :pre_chat_form_response, :assign_to_agent, :conversation_resolved, :skip_conversation_reopen, :should_show_message_on_chat, :is_ai_nudge], coder: JSON
  ...
  def valid_first_reply?
    return false unless valid_first_reply_conditions?
    return false if conversation.first_reply_created_at.present?
    return false if conversation.messages.incoming.count.zero?

    incoming_message = conversation.messages.incoming.last
    return false if other_outgoing_messages?(incoming_message)

    true
  end

  def valid_first_reply_conditions?
    outgoing? && human_response? && !private?
  end

  def other_outgoing_messages?(incoming_message)
    conversation.messages.outgoing
                ...
                .where('created_at > ?', incoming_message.created_at)
                .count > 1
  end
