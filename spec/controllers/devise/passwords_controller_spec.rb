...
RSpec.describe 'Passwords Controller', type: :request do
  it 'forwards the requested redirect to password recovery' do
    ...
    post user_password_path, params: { email: user.email, redirect_url: redirect_url, sso_account_id: '42' }, as: :json

    expect(user).to have_received(:send_reset_password_instructions).with(redirect_url: redirect_url, sso_account_id: '42')
