...
RSpec.describe 'Inboxes API', type: :request do
  ...
  describe 'GET /api/v1/account/{account.id}/inboxes' do
    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        get "/api/v1/account/#{account.id}/inboxes"
  ...
  describe 'DELETE /api/v1/account/{account.id}/inboxes/:id' do
    let(:inbox) { create(:inbox, account: account) }

    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        delete "/api/v1/account/#{account.id}/inboxes/#{inbox.id}"
    ...
    context 'when it is an authenticated user' do
      ...
      it 'is unable to delete inbox of another account' do
        ...
        delete "/api/v1/account/#{account.id}/inboxes/#{other_inbox.id}",
               headers: admin.create_new_auth_token,
               as: :json
  ...
  describe 'PATCH /api/v1/account/{account.id}/inboxes/:id' do
    let(:inbox) { create(:inbox, account: account) }

    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        patch "/api/v1/account/#{account.id}/inboxes/#{inbox.id}"
