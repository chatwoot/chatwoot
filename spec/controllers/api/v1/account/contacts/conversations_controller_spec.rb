...
RSpec.describe '/api/v1/account/{account.id}/contacts/:id/conversations', type: :request do
  ...
  describe 'GET /api/v1/account/{account.id}/contacts/:id/conversations' do
    context 'when unauthenticated user' do
      it 'returns unauthorized' do
        get "/api/v1/account/#{account.id}/contacts/#{contact.id}/conversations"
    ...
    context 'when user is logged in' do
      context 'with user as administrator' do
        it 'returns conversations from all inboxes' do
          get "/api/v1/account/#{account.id}/contacts/#{contact.id}/conversations", headers: admin.create_new_auth_token
      ...
      context 'with user as agent' do
        it 'returns conversations from the inboxes which agent has access to' do
          get "/api/v1/account/#{account.id}/contacts/#{contact.id}/conversations", headers: agent.create_new_auth_token
      ...
      context 'with user as unknown role' do
        it 'returns conversations from no inboxes' do
          get "/api/v1/account/#{account.id}/contacts/#{contact.id}/conversations", headers: unknown.create_new_auth_token
