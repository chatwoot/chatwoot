...
RSpec.describe 'Inbox Member API', type: :request do
  ...
  describe 'POST /api/v1/account/{account.id}/inbox_members' do
    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        post "/api/v1/account/#{account.id}/inbox_members"
  ...
  describe 'GET /api/v1/account/{account.id}/inbox_members/:id' do
    let(:inbox_member) { create(:inbox_member, inbox: inbox) }

    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        get "/api/v1/account/#{account.id}/inbox_members/#{inbox_member.id}"
    ...
    context 'when it is an authenticated user' do
      ...
      it 'returns inbox member' do
        get "/api/v1/account/#{account.id}/inbox_members/#{inbox.id}",
            headers: agent.create_new_auth_token,
            as: :json
