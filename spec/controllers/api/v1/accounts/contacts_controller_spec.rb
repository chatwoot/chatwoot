...
RSpec.describe 'Contacts API', type: :request do
  ...
  describe 'POST /api/v1/accounts/{account.id}/contacts/:id/block' do
    let!(:contact) { create(:contact, account: account) }

    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        post "/api/v1/accounts/#{account.id}/contacts/#{contact.id}/block"
    ...
    context 'when it is an authenticated user' do
      ...
      it 'prevents blocking contact of another account' do
        other_account = create(:account)
        other_contact = create(:contact, account: other_account)

        post "/api/v1/accounts/#{account.id}/contacts/#{other_contact.id}/block",
             headers: admin.create_new_auth_token
  ...
  describe 'POST /api/v1/accounts/{account.id}/contacts/:id/unblock' do
    let!(:contact) { create(:contact, account: account, blocked: true) }

    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        post "/api/v1/accounts/#{account.id}/contacts/#{contact.id}/unblock"
    ...
    context 'when it is an authenticated user' do
      ...
      it 'prevents unblocking contact of another account' do
        other_account = create(:account)
        other_contact = create(:contact, account: other_account, blocked: true)

        post "/api/v1/accounts/#{account.id}/contacts/#{other_contact.id}/unblock",
             headers: admin.create_new_auth_token
