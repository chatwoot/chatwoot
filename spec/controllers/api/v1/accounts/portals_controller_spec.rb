...
RSpec.describe 'Api::V1::Accounts::Portals', type: :request do
  ...
  describe 'PUT /api/v1/accounts/{account.id}/portals/{portal.slug}' do
    ...
    context 'when it is an authenticated user' do
      ...
      it 'attaches logo when both blob_id and blob_key are valid' do
      ...
      it 'does not attach logo when blob_key is missing' do
        file = fixture_file_upload(Rails.root.join('spec/assets/avatar.png'), 'image/png')
      ...
      it 'does not attach logo when blob_key is incorrect (prevents IDOR via enumeration)' do
        file = fixture_file_upload(Rails.root.join('spec/assets/avatar.png'), 'image/png')
      ...
      it 'does not attach logo when blob is already attached to another record' do
        file = fixture_file_upload(Rails.root.join('spec/assets/avatar.png'), 'image/png')
        ...
        # Attach blob to another portal first
        other_portal = create(:portal, slug: 'other-portal', name: 'Other Portal', account_id: account.id)
        other_portal.logo.attach(ActiveStorage::Blob.find(blob['blob_id']))
