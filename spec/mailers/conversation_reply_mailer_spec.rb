...
RSpec.describe ConversationReplyMailer do
  describe 'reply' do
    ...
    context 'when channel email address is available' do
      let(:email_channel) { create(:channel_email, account: account, email: 'noreply@chatwoot.com') }
      ...
      it 'set reply to email address as channel email address' do
        expect(mail.from).to eq([email_channel.email])
