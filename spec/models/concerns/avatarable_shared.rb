...
shared_examples_for 'avatarable' do
  ...
  context 'when avatarable has an email attribute' do
    it 'enques job when email is changed on avatarable create' do
      ...
      if avatarable.has_attribute?(:email)
      ...
      avatarable.save!
      expect(Avatar::AvatarFromGravatarJob).to have_been_enqueued.with(avatarable, avatarable.email) if avatarable.has_attribute?(:email)
    end
