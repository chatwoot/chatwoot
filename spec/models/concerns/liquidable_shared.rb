...
shared_examples_for 'liqudable' do
  ...
  context 'when liquid is present in email content' do
    let(:contact) { create(:contact, name: 'john', email: 'john@example.com') }
    let(:email_channel) { create(:channel_email) }
    let(:inbox) { create(:inbox, channel: email_channel) }
    let(:conversation) { create(:conversation, contact: contact, inbox: inbox) }

    context 'when message is outgoing with email content' do
      let(:message) { build(:message, conversation: conversation, message_type: 'outgoing') }

      it 'replaces liquid variables in email html_content' do
        message.content = 'Hello {{contact.name}}'
        message.content_attributes = {
          email: {
            html_content: {
              full: '<p>Hello {{contact.name}}</p>',
              reply: '<p>Hello {{contact.name}}</p>'
            },
            text_content: {
              full: 'Hello {{contact.name}}',
              reply: 'Hello {{contact.name}}'
            }
          }
        }
        message.save!

        expect(message.content_attributes[:email][:html_content][:full]).to eq '<p>Hello John</p>'
        expect(message.content_attributes[:email][:html_content][:reply]).to eq '<p>Hello John</p>'
        expect(message.content_attributes[:email][:text_content][:full]).to eq 'Hello John'
        expect(message.content_attributes[:email][:text_content][:reply]).to eq 'Hello John'
      end

      it 'replaces liquid variables with contact email' do
        message.content = 'Email: {{contact.email}}'
        message.content_attributes = {
          email: {
            html_content: {
              full: '<p>Email: {{contact.email}}</p>'
            }
          }
        }
        message.save!

        expect(message.content_attributes[:email][:html_content][:full]).to eq '<p>Email: john@example.com</p>'
      end

      it 'handles missing email content gracefully' do
        message.content = 'Hello {{contact.name}}'
        message.content_attributes = {}
        expect { message.save! }.not_to raise_error
      end

      it 'handles broken liquid syntax in email content' do
        message.content = 'Hello'
        message.content_attributes = {
          email: {
            html_content: {
              full: '<p>{{contact.name}  {{invalid}}</p>'
            }
          }
        }
        message.save!

        expect(message.content_attributes[:email][:html_content][:full]).to eq '<p>{{contact.name}  {{invalid}}</p>'
      end
    end

    context 'when message is incoming with email content' do
      let(:message) { build(:message, conversation: conversation, message_type: 'incoming') }

      it 'does not process liquid in email content' do
        message.content = 'Hello {{contact.name}}'
        message.content_attributes = {
          email: {
            html_content: {
        ...
        expect(message.content_attributes[:email][:html_content][:full]).to eq '<p>Hello {{contact.name}}</p>'
      end
    end
  end
end
