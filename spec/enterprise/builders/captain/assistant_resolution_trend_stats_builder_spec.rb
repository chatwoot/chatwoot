...
RSpec.describe Captain::AssistantResolutionTrendStatsBuilder do
  subject(:metrics) { described_class.new(assistant, 'this_month', 0).metrics }
  ...
  around do |example|
    travel_to(Time.zone.parse('2025-06-30 12:00:00')) { example.run }
  end
  ...
  it 'anchors calendar buckets to the viewer timezone' do
    ...
    timezone_metrics = described_class.new(assistant, 'this_month', 5.5).metrics
