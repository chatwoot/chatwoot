...
RSpec.describe 'Twilio::VoiceController', type: :request do
  ...
  describe 'POST /twilio/voice/call/:phone' do
    ...
    # Existing inboxes carry no recording key in provider_config; they must keep recording.
    it 'records the conference from the start on an inbox that never touched the setting' do
      conversation = create(:conversation, account: account, inbox: inbox)
