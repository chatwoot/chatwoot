...
RSpec.describe ConversationMonitors::BackfillJob do
  ...
  it 'enumerates only the initial seven-day population in the owning account' do
    ...
    expect(monitor.backfill.reload.enumerated_at).to be_present
  ...
  it 'resumes a bounded scan and tolerates duplicate jobs without duplicate memberships' do
    ...
    stub_const('ConversationMonitors::BackfillJob::BATCH_SIZE', 2)

    described_class.perform_now(monitor.id)
    expect(monitor.backfill.reload.enumerated_at).to be_nil
  ...
  it 'recovers durable work after enqueue fails after a batch commits' do
    ...
    expect { described_class.perform_now(monitor.id) }.to raise_error(Redis::CannotConnectError)
  ...
  it 'retains the unfinished cursor while collection is disabled' do
    ...
    expect(monitor.backfill.reload.cursor).to eq(0)
    expect(monitor.backfill.enumerated_at).to be_nil
