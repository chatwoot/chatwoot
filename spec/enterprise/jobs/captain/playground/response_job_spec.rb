...
RSpec.describe Captain::Playground::ResponseJob, type: :job do
  ...
  it 'generates a legacy response and broadcasts it to the requesting user' do
    chat_service = instance_double(Captain::Llm::AssistantChatService)
    allow(Captain::Llm::AssistantChatService).to receive(:new)
      .with(assistant: assistant, source: 'playground')
    ...
    expect(ActionCableBroadcastJob).to receive(:perform_later).with(
      ...
      { content: 'Assistant response', account_id: account.id, request_id: request_id }
    )
