...
RSpec.describe Crm::MetaCapi::PayloadBuilder do
  ...
  describe '.build' do
    it 'builds a business_messaging event carrying the ctwa_clid, waba id and external event_id' do
      card = build_card(value_cents: 50_000, external_id: 'EXT-9')

      event = described_class.build(card: card, event_name: 'Purchase', event_type: 'won', event_id: 'crm-1-won-42',
                                    attribution: { ctwa_clid: 'CLID1', waba_id: 'WABA1' })

      expect(event['event_name']).to eq('Purchase')
      expect(event['event_id']).to eq('crm-1-won-42')
    ...
    it 'omits value and currency for a zero-value funnel movement' do
      ...
      event = described_class.build(card: card, event_name: 'QualifiedLead', event_type: 'moved', event_id: 'crm-1-moved-7',
                                    attribution: { ctwa_clid: 'CLID1', waba_id: nil })

      expect(event).not_to have_key('custom_data')
      expect(event['user_data']).to eq('ctwa_clid' => 'CLID1')
    end

    it 'anchors event_time to closed_at for result events and echoes the caller event_id' do
      closed = 2.days.ago.change(usec: 0)
      card = build_card(status: :won, value_cents: 100)
      card.update!(closed_at: closed)

      event = described_class.build(card: card, event_name: 'Purchase', event_type: 'won', event_id: 'crm-1-won-99',
                                    attribution: { ctwa_clid: 'CLID1', waba_id: 'WABA1' })

      expect(event['event_time']).to eq(closed.to_i)
      expect(event['event_id']).to eq('crm-1-won-99')
    end
    ...
    it 'anchors event_time to entered_stage_at for movement events' do
      ...
      event = described_class.build(card: card, event_name: 'QualifiedLead', event_type: 'moved', event_id: 'crm-1-moved-3',
                                    attribution: { ctwa_clid: 'CLID1', waba_id: 'WABA1' })
