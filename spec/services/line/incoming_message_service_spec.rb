...
describe Line::IncomingMessageService do
  ...
  describe '#perform' do
    ...
    context 'when valid receive input_select response' do
      before do
        line_bot = double
        line_user_profile = double
        allow(Line::Bot::Client).to receive(:new).and_return(line_bot)
      ...
      it 'set submitted_values when select option to response' do
        described_class.new(inbox: line_channel.inbox, params: params).perform

        expect(line_channel.inbox.conversations.length).to eq(1)

        conversation = line_channel.inbox.conversations.first
        expect(conversation.messages.length).to eq(2)
        expect(conversation.messages.first.submitted_values.length).to eq(1)
        expect(conversation.messages.first.submitted_values[0]['title']).to eq('hello')
        expect(conversation.messages.first.submitted_values[0]['value']).to eq('Hello, world')
      end
