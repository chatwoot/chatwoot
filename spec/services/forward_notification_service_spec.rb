...
RSpec.describe ForwardNotificationService do
  ...
  before do
    ...
    # Mock the account's whatsapp_channels to return our mock channel
    allow(account).to receive(:whatsapp_channels).and_return([whatsapp_channel])
  end
  ...
  describe '#send_whatsapp_notifications' do
    ...
    context 'when no WhatsApp channel exists' do
      before do
        # Mock the account to return no WhatsApp channels
        allow(account).to receive(:whatsapp_channels).and_return([])
        allow(Rails.logger).to receive(:warn)
      end
      ...
      it 'logs a warning and returns early' do
        ...
        expect(Rails.logger).to have_received(:warn).with("No WhatsApp channels found for account #{account.id}")
