...
RSpec.describe V2::Reports::BotMetricsBuilder do
  ...
  let!(:conversation) { create(:conversation, account: inbox.account, inbox: inbox, created_at: 2.days.ago) }
  ...
  before do
    ...
    create(:message, account: inbox.account, conversation: conversation, created_at: 2.days.ago, message_type: 'outgoing')
    create(:reporting_event, account_id: inbox.account.id, name: 'conversation_bot_resolved', conversation_id: conversation.id,
                             created_at: 2.days.ago)
    create(:reporting_event, account_id: inbox.account.id, name: 'conversation_bot_handoff',
                             conversation_id: conversation.id, created_at: 2.days.ago)
  end
