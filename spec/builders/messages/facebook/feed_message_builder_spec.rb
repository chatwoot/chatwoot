...
describe Messages::Facebook::FeedMessageBuilder do
  subject(:message_builder) { described_class.new(response, facebook_channel.inbox).perform }

  before do
    stub_request(:post, /graph.facebook.com/)
    allow(Messages::Facebook::FeedActivityMessageBuilder)
      .to receive(:new)
  ...
  describe '#perform' do
    it 'creates contact and message for the facebook inbox' do
      ...
      expect(message.content).to eq('comment message')
    end
    ...
    context 'when lock to single conversation' do
      subject(:mocked_message_builder) do
        described_class.new(response, facebook_channel.inbox).perform
      end

      let!(:mocked_message_object) { build(:mocked_feed_message, sender_id: contact_inbox.source_id).to_json }
      let!(:response) { Integrations::Facebook::FeedMessageParser.new(mocked_message_object) }
