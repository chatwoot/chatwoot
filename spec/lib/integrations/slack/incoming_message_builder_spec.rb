  ...
  describe '#perform' do
    ...
    context 'Message' do
      it 'should create message' do
        ...
        builder = Integrations::Slack::IncomingMessageBuilder.new(message_params)
        response = builder.perform
        expect(conversation.messages.count).to eql(messages_count + 1)  
      end

      it 'should not create message for invalid event type' do
        messages_count = conversation.messages.count
      ...
      it 'should not create message for invalid event name' do
        messages_count = conversation.messages.count
