...
RSpec.describe Internal::CheckNewVersionsJob do
  ...
  it 'updates the latest chatwoot version in redis' do
    ...
    # Set time to match the designated hour
    installation_id = 'test-uuid'
    allow(ChatwootHub).to receive(:installation_identifier).and_return(installation_id)
    designated_hour = Digest::MD5.hexdigest(installation_id).hex % 24
    travel_to Time.zone.parse("2024-01-15 #{designated_hour}:00:00 UTC")
