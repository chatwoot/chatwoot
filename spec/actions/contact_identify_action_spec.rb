...
describe ContactIdentifyAction do
  ...
  describe '#perform' do
    it 'updates the contact' do
      ...
      expect(contact.custom_attributes).to eq({ 'ai_enabled' => true, 'test' => 'new test', 'test1' => 'test1', 'test2' => 'test2' })
    ...
    it 'ensures ai_enabled defaults to true when not present in contact or params' do
      contact = create(:contact, account: account, custom_attributes: {})
      params = { name: 'test' }
      result = described_class.new(contact: contact, params: params).perform
      expect(result.custom_attributes['ai_enabled']).to eq(true)
    end
  end
end
