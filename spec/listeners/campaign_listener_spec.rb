...
describe CampaignListener do
  ...
  let!(:event) { Events::Base.new('campaign_triggered', Time.zone.now, contact_inbox: contact_inbox, event_info: {campaign_id: campaign.display_id}) }
  ...
  describe '#campaign_triggered' do
    ...
    context 'when params contain campaign id' do
      it 'triggers campaign conversation builder' do
        expect(Campaigns::CampaignConversationBuilder).to receive(:new).with({ contact_inbox: contact_inbox.id,
      campaign_display_id: campaign.display_id}).once
    ...
    context 'when params does not contain campaign id' do
      it 'does not trigger campaign conversation builder' do
        event = Events::Base.new('campaign_triggered', Time.zone.now, contact_inbox: contact_inbox, event_info: {})
