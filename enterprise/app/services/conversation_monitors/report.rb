class ConversationMonitors::Report
  ...
  def timeseries
    counts = bucket_counts
    {
      ...
      buckets: @buckets.ranges.each_with_index.map do |range, index|
        { start: range.begin.to_i, end: range.end.to_i, count: counts.fetch(index, 0), covered: covered?(range) }
      end
    }
  end
  ...
  def covered?(range)
    @gaps ||= @monitor.resumptions.incomplete.to_a
    range.begin >= @monitor.history_since && @gaps.none? { |gap| gap.started_at < range.end && gap.ended_at > range.begin }
  end
  ...
  def bucket_counts
    scope = @monitor.matched_conversations.where(created_at: @buckets.since...@buckets.until_time).select(:created_at)
