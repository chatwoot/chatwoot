...
module Captain::Routines::AgentTask
  ...
  def run_agent(input:, **options)
    ...
    agent_context = options.fetch(:context, {})
    ...
    result = runner.run(input, context: agent_context, max_turns: options.fetch(:max_turns, 10))
    return agent_error(result.error, agent_context: result.context) if result.error

    agent_response(result)
  rescue StandardError => e
    agent_error(e, agent_context: agent_context)
  end
  ...
  def agent_error(error, agent_context: nil)
    message = error.respond_to?(:message) ? error.message : error.to_s
    { error: message, error_code: 422, agent_context: agent_context }.compact
  end
end
