class Captain::AssistantMigration::InstructionClassifier < Captain::BaseTaskService
  ...
  def system_prompt
    <<~PROMPT
      ...
      - Keep confidence and review reasoning only in the schema's confidence and review_reason fields. Never put
        confidence labels, review notes, bracketed reviewer comments, or schema labels inside migrated text values,
        scenario titles, scenario descriptions, scenario instructions, conversation messages, or classification notes.
      ...
      - Do not create scenario candidates for simple routing triggers such as "user asks for a human", "user is
        interested in sales", "order status should use an unavailable order tool", or "immediately hand off this
        category" unless there is a concrete intake, qualification, troubleshooting, or fulfillment workflow to run.
