...
# conversation_outcomes. The cohort for a window is the demand that entered it
# (episodes started inside the window), so every rate shares one denominator
# and an episode never migrates between windows as it progresses.
#
# The table stores only facts (see ConversationOutcome); every
...
class Captain::AssistantOutcomeStatsBuilder
  ...
  # A later episode closes this one at its demand boundary. Resolutions count
  # as durable when no new episode starts during the follow-up window.
  ...
  def outcomes_scope(range)
    assistant.conversation_outcomes.where(started_at: range)
  end
  ...
  def window_clause(range)
    "started_at >= #{quote(range.first)} AND started_at <= #{quote(range.last)}"
  end
