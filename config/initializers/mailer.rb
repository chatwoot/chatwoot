Rails.application.configure do
  ...
  config.action_mailer.default_url_options = { host: ENV.fetch('FRONTEND_URL', nil) } if ENV['FRONTEND_URL'].present?
  ...
  smtp_settings[:domain] = ENV.fetch('SMTP_DOMAIN', nil) if ENV['SMTP_DOMAIN'].present?
  ...
  smtp_settings[:openssl_verify_mode] = ENV.fetch('SMTP_OPENSSL_VERIFY_MODE', nil) if ENV['SMTP_OPENSSL_VERIFY_MODE'].present?
  ...
  config.action_mailer.delivery_method = :letter_opener if Rails.env.development? && ENV.fetch('LETTER_OPENER', nil)
