class Rack::Attack
  ...
  throttle('/api/v1/accounts/:account_id/whapi_channels', limit: ENV.fetch('RATE_LIMIT_WHAPI_CHANNEL_CREATE_PER_HOUR', '10').to_i, period: 1.hour) do |req|
  ...
  throttle('/api/v1/accounts/:account_id/whapi_channels/:id/qr_code_generate', limit: ENV.fetch('RATE_LIMIT_WHAPI_QR_GENERATE_PER_HOUR', '20').to_i, period: 1.hour) do |req|
    match_data = %r{/api/v1/accounts/(?<account_id>\d+)/whapi_channels/(?<inbox_id>\d+)/qr_code}.match(req.path)
    match_data[:account_id] if match_data.present? && req.get?
  end
