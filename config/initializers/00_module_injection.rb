...
#
# NOTE: this is now generic module-injection scaffolding. The upstream
# "enterprise edition" it used to load is removed; `ChatwootApp.extensions`
# returns an empty list, so every `prepend_mod_with`/`include_mod_with` call
# below is a harmless no-op loop. Do not delete this file — the public
# `prepend_mod_with`/`include_mod_with`/`extend_mod_with` methods are still
# called across the app.
#
...
Module.prepend(InjectModuleExtension)
