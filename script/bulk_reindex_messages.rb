...
Message.find_in_batches(batch_size: BATCH_SIZE) do |batch|
  ...
  if batch_count % JOBS_PER_MINUTE == 0
    elapsed = Time.now - start_time
    progress = (batch_count.to_f / total_batches * 100).round(2)
    queue_size = Sidekiq::Queue.new('bulk_reindex_low').size

    puts "[#{Time.now.strftime('%Y-%m-%d %H:%M:%S')}] Progress: #{batch_count}/#{total_batches} (#{progress}%)"
...
puts "Total time: #{((Time.now - start_time) / 3600).round(2)} hours"
puts '=' * 80
